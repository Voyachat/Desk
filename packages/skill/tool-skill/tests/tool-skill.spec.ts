...
import { createUserMessage, CallId, type Message } from '@voyaseek-ai/dsh-llm'
import { createScope, type Scope } from '@voyaseek-ai/dsh-scope'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@voyaseek-ai/dsh-session'
...
import AgentRegistry, { agentEvents, Inbox, type Agent, type PreStepDecision } from '@voyaseek-ai/dsh-agent'
...
describe('dsh-tool-skill', () => {
  ...
  it('loads a skill for the calling agent cwd', async () => {
    ...
    await writeSkill(join(project, '.voyaseek/skills'), 'project-skill', 'Project skill', 'Project instructions.')
    const ctx = await setup(home)
    ...
    expect(result.value).toEqual({
      name: 'project-skill',
      provider: 'filesystem',
      resourceBase: { kind: 'directory', path: join(project, '.voyaseek/skills/project-skill') },
    ...
    expect(block.text).toBe([
      ...
      '<skill_resources>',
      `Base directory for this skill: ${join(project, '.voyaseek/skills/project-skill')}`,
  ...
  it('returns isError for unknown, invalid, and model-disabled skills', async () => {
    ...
    await writeSkill(join(home, '.voyaseek/skills'), 'hidden-skill', 'Hidden skill', 'Hidden instructions.')
    await writeFile(join(home, '.voyaseek/skills/hidden-skill/SKILL.md'), '---\nname: hidden-skill\ndescription: Hidden skill\ndisable-model-invocation: true\n---\n\nHidden instructions.\n')
    const ctx = await setup(home)
    ctx.skills.register({
      name: 'model-only-skill',
      description: 'Model-only skill',
