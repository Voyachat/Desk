...
This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@voyaseek-ai/dsh-skill`; the durable session catalogs and model-facing loader tool remain in `@voyaseek-ai/dsh-tool-skill`.
...
| `dshHome` | `$VOYASEEK_HOME` or `~/.voyaseek` | Voyaseek Harness config root resolved by [`@voyaseek-ai/dsh-home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
...
| Rank | Source | Path |
|---|---|---|
| 100 | `project-dsh` | `<projectRoot>/.voyaseek/skills` |
