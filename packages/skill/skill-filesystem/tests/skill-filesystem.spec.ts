...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@voyaseek-ai/dsh-fs'
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.voyaseek/skills'), 'same', 'user dsh skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.voyaseek/skills'), 'same', 'project dsh skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.voyaseek/skills/.system'), 'hidden-system', 'hidden system')
    ...
    await writeSkill(join(noGit, '.voyaseek/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.voyaseek/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.voyaseek/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.voyaseek/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.voyaseek/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.voyaseek/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.voyaseek/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.voyaseek/skills/broken-link'))
    await symlink('/dev/null', join(home, '.voyaseek/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { dshHome: join(home, '.voyaseek'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.voyaseek/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.voyaseek/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousDshHome = process.env.VOYASEEK_HOME
    ...
    try {
      process.env.VOYASEEK_HOME = join(envHome, '.voyaseek')
      ...
      await writeSkill(join(envHome, '.voyaseek/skills'), 'env-skill', 'Env skill')
      ...
      process.env.VOYASEEK_HOME = join(envHome, 'empty-dsh')
      ...
      if (previousDshHome === undefined) {
        ...
        process.env.VOYASEEK_HOME = previousDshHome
      }
