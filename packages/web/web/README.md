...
| Package | Role |
|---|---|
| `@voyaseek-ai/dsh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@voyaseek-ai/dsh-web-search-exa` | Search provider: Exa |
| `@voyaseek-ai/dsh-web-search-perplexity` | Search provider: Perplexity |
| `@voyaseek-ai/dsh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@voyaseek-ai/dsh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
