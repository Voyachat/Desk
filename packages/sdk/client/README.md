# @voyaseek-ai/dsh-sdk-client

English | [中文](README.zh.md)

The TypeScript client SDK for driving a Voyaseek Harness runtime as a subprocess over stdio JSON-RPC — the design twin of the [Python SDK](../../../python/README.md) (`voyaseek-harness`), sharing the same runtime peer, protocol, and layering: `VoyaseekHarness` is the high-level owned-run API, `HarnessClient` the lower-level protocol client. The package root enumerates the consumer interface: the two client layers, caller-facing types, and `JsonRpcResponseError`; source modules, normalization helpers, and subscription-delivery machinery are not consumer imports. A pure library: it registers nothing on a Cordis context; the runtime process it spawns is a complete harness whose composition its own `cordis.yml` decides.
...
```ts
import { VoyaseekHarness } from '@voyaseek-ai/dsh-sdk-client'

await using harness = new VoyaseekHarness({
