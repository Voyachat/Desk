/**
 * High-level run API over {@link HarnessClient}: `VoyaseekHarness` owns one
 ...
 * Mirrors the Python SDK's `VoyaseekHarness`/`Session` pair.
 *
 * @module @voyaseek-ai/dsh-sdk-client/api
 */
...
import type { ContentBlock, VoyaseekHarnessOptions, HarnessClientOptions, HarnessNotification, RunResult } from './types.ts'

/**
 * Reusable SDK for running Voyaseek Harness agent turns in a runtime
 ...
 * child is reaped.
 */
export class VoyaseekHarness implements AsyncDisposable {
  ...
  constructor(options: VoyaseekHarnessOptions) {
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: VoyaseekHarness, readonly id: string) {}
