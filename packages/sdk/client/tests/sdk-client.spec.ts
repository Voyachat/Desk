...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): VoyaseekHarness {
  const harness = new VoyaseekHarness({ launch: fakeLaunch(env, extra) })
...
describe('VoyaseekHarness', () => {
  ...
  it('supports await using disposal', async () => {
    let captured: VoyaseekHarness
    {
      await using harness = new VoyaseekHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('voyaseek-harness-sdk-runtime')
