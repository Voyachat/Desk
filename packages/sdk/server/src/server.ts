 ...
 *
 * @module @voyaseek-ai/dsh-sdk-jsonrpc-server/server
 */

import type { Context } from '@voyaseek-ai/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@voyaseek-ai/dsh-agent'
import { createUserMessage } from '@voyaseek-ai/dsh-llm'
import { carrierKeyOf, type Scoped } from '@voyaseek-ai/dsh-scope'
import { SessionId } from '@voyaseek-ai/dsh-session'
import type SubagentRuntime from '@voyaseek-ai/dsh-subagent'
import type { SubagentRunEndInfo } from '@voyaseek-ai/dsh-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    return { serverInfo: { name: 'voyaseek-harness-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown Voyaseek Harness SDK runtime method: ${method}`)
    }
  }
  ...
  private async createSession(sessionId: string): Promise<SessionRecord> {
    ...
    // (@voyaseek-ai/dsh-agent-presets README, "Composing a child agent").
