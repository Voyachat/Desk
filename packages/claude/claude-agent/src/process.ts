 ...
 * Agent SDK's custom-spawn process interface. The adapter mirrors the
...
/* jscpd:ignore-start -- the spawn-spec translation is the stable subprocess
 * seam contract shared with the one-shot subagent package; importing it from
 * there would couple this decoupled package to the subagent package. */
/**
...
export function claudeSpawnSpec(
  ...
  if (options.cwd === undefined || options.cwd.length === 0) {
    throw new Error('claude-agent: SDK spawn request omitted its workspace')
  }
...
/* jscpd:ignore-start -- same seam-contract duplication rationale as above. */
/**
