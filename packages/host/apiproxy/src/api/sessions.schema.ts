...
import { z } from 'zod'
import type { SessionEvent, SessionId } from '@voyaseek-ai/dsh-session/types'
...
import type { AttachmentIdType, ImageAttachmentLimits, ImageAttachmentRef } from '@voyaseek-ai/dsh-attachment'
...
/** session.fork request payload (atSeq anchors the cut; present agentRuntime selects the child driver). */
...
/** session.fork response value (the child identity and effective alternative runtime). */
