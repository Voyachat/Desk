...
`session.fork` 将可选事件锚点映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。发布后的子会话会先继承源会话的种子历史、cwd、日志中最新的 `ModelSelection` 及谱系，再加入源 Workspace。省略 `agentRuntime` 时继承源驱动；显式提供 id 时选择另一个已注册驱动，`''` 则选择本机循环。Runtime 变化时，Host 会把 `agent/runtime/switched` 加入种子、重置提供方私有续接状态；若继承的日志模型不在目标 Runtime 允许的子集中，则回退到目标 Runtime 默认模型。如果附加到 Workspace 失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了为何锚点要映射到该 `turn/end`。
...
`llm.probeRuntime` 只接受已保存的提供方、模型与 Runtime 身份。Host 会在所属适配器内部解析凭据，并返回 `unsupported`、`unverified`、`reachable` 或 `failed`，最多附带稳定失败 code；凭据值与提供方响应正文都不会跨越 wire。
...
结构化条目使用独立的 `memory.list`／`forget`／`clear` 领域。三个方法都仅限 loopback，且绝不返回 Host SQLite 路径。
