 ...
 * @module @voyaseek-ai/dsh-sandbox-local
 */
...
} from '@voyaseek-ai/node-addon-landlock-run'
import { Context } from '@voyaseek-ai/cordis'
import z from '@voyaseek-ai/schemastery'
import { assertNever } from '@voyaseek-ai/dsh-llm'
import { SandboxProvider, SandboxUnavailableError } from '@voyaseek-ai/dsh-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@voyaseek-ai/dsh-sandbox'
import type { SessionId } from '@voyaseek-ai/dsh-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@voyaseek-ai/dsh-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@voyaseek-ai/dsh-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@voyaseek-ai/dsh-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@voyaseek-ai/dsh-sandbox-windows-acl/src/runner.ts'))
