...
对于根 transport 执行（`exec.parent` 缺省），工具还会把运行投影到调用 Agent 的 Session：`start()` 返回后，run-start 记录 `run.id`、来源 `callId` 与展示名称；成员开始与结束按 `run.id` 筛选；只有 `run.result` 已取得且 `dispose()` 完全停稳后才追加 run-end。嵌套 transport 调用照常执行，但不写工作流记录。任一次 Session append 首次失败后，本运行会停止后续记录并只告警一次，留下空记录或合法连续前缀，同时不改变工具结果和清理。

Agent 恢复持久 Session 时，插件会折叠 Session 日志，为上一个进程遗留的每个开放成员和运行追加 `interrupted` 终点。普通启动不做收敛，已有终态的运行保持不变，同一恢复通知重复发生时不会在首次收敛后继续追加。恢复过程绝不执行日志中的脚本，也不会把不完整运行改成成功。
...
工具可见时，已生成的默认 [`workflow` schema](../../../docs/tool-catalog.md#voyaseek-aidsh-tool-workflow) 包含完整的 JavaScript 钩子与元数据约定；`toolName` 可以重命名该定义，模型会提交脚本、元数据和可选 args。
...
- **恢复是显式的从头运行，不是 checkpoint 续跑**：任意工作流 JavaScript 没有持久指令位置或幂等保证。进程丢失后，旧记录会结算为已中断；只有用户发起重试才会创建新运行。
- **持久记录只覆盖顶层调用**：嵌套 Code Mode dispatch 不记录；记录故障会刻意退化为不完整前缀，而不改变执行。Agent Session 恢复时，不完整记录才会变为可恢复状态。
