...
async function setup(
  config?: { toolName?: string; maxResultChars?: number },
...
describe('dsh-tool-workflow', () => {
  ...
  it('records one top-level run and its members in the calling Session after cleanup', async () => {
    ...
    expect(session.events.map(event => [event.type, event.data])).toEqual([
      ['tool-workflow/run-start', { runId: 'run-1', callId: 'call-1', name: 'audit' }],
  ...
  it('durably interrupts open runs on resume without replaying workflow code', async () => {
    const { ctx, engine, parent, session } = await setup()
    const runId = WorkflowRunId('orphaned')
    session.append('tool-workflow/run-start', {
      runId, callId: CallId('call-orphaned'), name: 'orphaned',
    })
    session.append('tool-workflow/agent-start', {
      runId, seq: 1, label: 'finished', childId: SessionId('child-finished'),
    })
    session.append('tool-workflow/agent-end', { runId, seq: 1, outcome: 'completed' })
    session.append('tool-workflow/agent-start', {
      runId, seq: 2, label: 'open', childId: SessionId('child-open'),
    })

    agentEvents(ctx, parent).emit('agent/session-start', { source: 'startup' })
    expect(session.events.at(-1)?.type).toBe('tool-workflow/agent-start')
    agentEvents(ctx, parent).emit('agent/session-start', { source: 'resume' })

    expect(session.events.slice(-2).map(event => [event.type, event.data])).toEqual([
      ['tool-workflow/agent-end', { runId: 'orphaned', seq: 2, outcome: 'interrupted' }],
      ['tool-workflow/run-end', { runId: 'orphaned', stopReason: 'interrupted' }],
    ])
    expect(engine.requests).toEqual([])
    const recoveredLength = session.events.length
    agentEvents(ctx, parent).emit('agent/session-start', { source: 'resume' })
    expect(session.events).toHaveLength(recoveredLength)
  })

  it('restarts an explicitly recoverable run from its exact logged source call', async () => {
    const { ctx, engine, parent, session } = await setup(undefined, { commands: true })
    const sourceCallId = CallId('call-source')
    const sourceArgs = { script: 'return args.value', meta: META, args: { value: 7 } }
    session.append('tool/call', {
      turn: 1, step: 1, callId: sourceCallId, name: 'workflow', arguments: JSON.stringify(sourceArgs),
    })
    session.append('tool-workflow/run-start', {
      runId: WorkflowRunId('orphaned'), callId: sourceCallId, name: META.name,
    })
    session.append('tool-workflow/run-end', {
      runId: WorkflowRunId('orphaned'), stopReason: 'interrupted',
    })

    const pending = ctx.commands.execute(
      parent,
      '/workflow-retry orphaned',
      new AbortController().signal,
    )
    await vi.waitFor(() => { expect(engine.requests).toHaveLength(1) })
    expect(engine.requests[0]).toMatchObject({ ...sourceArgs, parent })
    engine.settleRun(WorkflowRunId('run-1'), {
      value: 7, stopReason: 'completed', agentsStarted: 0,
    })

    await expect(pending).resolves.toMatchObject({
      result: {
        kind: 'success',
        text: expect.stringContaining('restarted from the beginning'),
      },
    })
    expect(session.events.filter(event => event.type === 'tool-workflow/run-start').at(-1)?.data)
      .toEqual({ runId: 'run-1', callId: sourceCallId, name: META.name })
  })
