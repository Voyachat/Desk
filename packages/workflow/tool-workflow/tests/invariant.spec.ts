...
import SessionStore, { SessionId, type Session } from '@voyaseek-ai/dsh-session'
import { CallId } from '@voyaseek-ai/dsh-llm'
import { WorkflowRunId, type WorkflowRunId as WorkflowRunIdType } from '@voyaseek-ai/dsh-workflow/types'
...
function runStart(runId: WorkflowRunIdType, name: string) {
  return { runId, callId: CallId(`call-${runId}`), name }
}
...
describe('durable workflow-record invariants', () => {
  it('accepts interleaved complete runs and an unfinished continuous prefix', async () => {
    ...
    session.append('tool-workflow/run-start', runStart(first, 'first'))
    session.append('tool-workflow/run-start', runStart(second, 'second'))
    ...
    session.append('tool-workflow/run-start', runStart(third, 'third'))
    session.append('tool-workflow/agent-start', {
    ...
    session.append('tool-workflow/run-start', runStart(WorkflowRunId('prefix'), 'prefix'))
  ...
  it('rejects a malformed candidate before commit and keeps the fold reusable', async () => {
    ...
    session.append('tool-workflow/run-start', runStart(runId, 'run'))
    const before = session.seq
    expect(() => session.append('tool-workflow/agent-end', {
      runId, seq: 1, outcome: 'completed',
    })).toThrow(expect.objectContaining<Partial<InvariantError>>({
      code: 'INVARIANT',
      packageName: '@voyaseek-ai/dsh-tool-workflow',
    }))
  ...
  const invalidCases: readonly [string, Mutation, RegExp][] = [
    ...
    ['empty run name', (session) => {
      session.append('tool-workflow/run-start', runStart(WorkflowRunId('empty-name'), ''))
    }, /name must be a non-empty string/],
    ['non-string run name', (session) => {
      session.append('tool-workflow/run-start', {
        ...runStart(WorkflowRunId('bad-name'), 'bad-name'), name: 1 as never,
      })
    }, /name must be a non-empty string/],
    ['empty source call id', (session) => {
      session.append('tool-workflow/run-start', {
        runId: WorkflowRunId('empty-call'), callId: CallId(''), name: 'empty-call',
      })
    }, /callId must be a non-empty string/],
    ['duplicate run', (session, runId) => {
      session.append('tool-workflow/run-start', runStart(runId, 'again'))
    }, /repeats run/],
    ['missing run', (session) => {
      session.append('tool-workflow/agent-start', {
  ...
  it('validates existing cold history while allowing an unfinished prefix', async () => {
    ...
    valid.append('tool-workflow/run-start', runStart(WorkflowRunId('valid'), 'valid'))
    ...
    broken.append('tool-workflow/run-start', runStart(WorkflowRunId('broken'), 'broken'))
