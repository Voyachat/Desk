...
For a root transport execution (`exec.parent` absent), the tool also projects the run into the calling Agent's Session: run-start records `run.id`, the originating `callId`, and the display name after `start()` returns; matching member starts and endings are filtered by `run.id`; run-end is appended only after `run.result` is available and `dispose()` has reached quiescence. Nested transport calls execute normally but write no workflow record. The first failed Session append disables later recording for that run, emits one warning, and leaves either no record or a legal continuous prefix without changing the tool result or cleanup.

When an Agent resumes a persisted Session, the plugin folds the Session log and appends `interrupted` endings for every member and run left open by the previous process. A normal startup does not reconcile records, terminal runs remain unchanged, and repeated resume notifications append nothing after the first recovery. Recovery never executes the logged script and never converts an incomplete run to success.
...
When visible, the generated default [`workflow` schema](../../../docs/tool-catalog.md#voyaseek-aidsh-tool-workflow) carries the complete JavaScript hook and metadata contract; `toolName` can rename the definition, and the model submits script, metadata, and optional args.
...
- **Recovery is an explicit from-start run, not a checkpoint resume** — arbitrary workflow JavaScript has no durable instruction pointer or idempotency guarantee. Process loss settles the old record as interrupted; only a person-initiated retry creates a new run.
- **Durable records cover top-level calls only** — nested Code Mode dispatches are not recorded, and a recording failure intentionally degrades to an incomplete prefix rather than changing execution. An incomplete record becomes recoverable when its Agent Session is resumed.
