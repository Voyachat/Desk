...
| Package | Role |
|---|---|
| `@voyaseek-ai/dsh-shell` (this) | Service Definition: abstract service + vocabulary types |
| `@voyaseek-ai/dsh-bash-local` | Service Provider: local subprocesses |
| `@voyaseek-ai/dsh-bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@voyaseek-ai/dsh-tool-bash` | the model-facing tool schemas over `ctx.shell` |
