...
会话遥测、直接反馈确认与 DeepSeek 提供方请求共用的匿名身份。`getOrCreateAnonymousUserId()` 返回一个限定于单个 harness home 的随机 UUID v4，并以裸行形式持久化到 `$VOYASEEK_HOME/.anonymous-user-id`（未设置 `VOYASEEK_HOME` 时为 `~/.voyaseek/.anonymous-user-id`）。OpenTelemetry 后端将其作为 Resource 的 `user.id` 上报；`/feedback` 在确认文本中包含同一个值；`dsh-llm-deepseek` 则通过 `x-voyaseek-harness-user-id` 发送该值，使接收系统无需独立生成身份即可关联记录。
...
- **没有跨 home 身份**：不同 `$VOYASEEK_HOME` 值之间无法关联。
