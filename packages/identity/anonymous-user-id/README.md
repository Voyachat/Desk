# @voyaseek-ai/dsh-anonymous-user-id

English | [中文](README.zh.md)

Shared anonymous identity for session telemetry, direct feedback acknowledgement, and DeepSeek provider requests. `getOrCreateAnonymousUserId()` returns a random UUID v4 scoped to one harness home, persisted as the bare line `$VOYASEEK_HOME/.anonymous-user-id` (`~/.voyaseek/.anonymous-user-id` when `VOYASEEK_HOME` is unset). The OpenTelemetry backend reports it as Resource `user.id`; `/feedback` includes the same value in its acknowledgement; and `dsh-llm-deepseek` sends it as `x-voyaseek-harness-user-id`, allowing the receiving systems to correlate records without independently generated identities.
...
- **No cross-home identity** — different `$VOYASEEK_HOME` values cannot be correlated.
