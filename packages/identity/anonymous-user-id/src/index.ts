 ...
 * harness home resolved by {@link resolveDshHome} (`$VOYASEEK_HOME` > `~/.voyaseek`),
 ...
 * machine: every process sharing one `$VOYASEEK_HOME` reports the same id, and
 ...
 *
 * @module @voyaseek-ai/dsh-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `VOYASEEK_HOME`; defaults to `process.env`. */
