...
await writeFileAtomic('/home/u/.voyaseek/settings.yaml', text, { mode: 0o600 })

// Read-modify-write against the same file from several processes.
await withFileLock('/home/u/.voyaseek/settings.yaml', async () => {
  await writeFileAtomic('/home/u/.voyaseek/settings.yaml', render(text), { mode: 0o600 })
})
```
