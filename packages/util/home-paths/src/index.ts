 ...
 *
 * @module @voyaseek-ai/dsh-home-paths
 */
...
/** Directory name for the default Voyaseek Harness home under the OS home. */
export const VOYASEEK_HOME_DIR_NAME = '.voyaseek'

/** Stable user-facing display form for the default Voyaseek Harness home. */
export const DEFAULT_VOYASEEK_HOME_DISPLAY = `~/${VOYASEEK_HOME_DIR_NAME}`

/** Environment variable that overrides the default Voyaseek Harness home. */
export const VOYASEEK_HOME_ENV = 'VOYASEEK_HOME'
...
/**
 * Resolve the default Voyaseek Harness home using Node's platform path rules.
 * @returns the absolute default harness home path.
 */
export function defaultDshHome(): string {
  return join(homedir(), VOYASEEK_HOME_DIR_NAME)
}
...
/**
 * Resolve the single-root Voyaseek Harness home.
 *
 * Precedence, highest first: an explicit configured path, `$VOYASEEK_HOME`, then
 * `~/.voyaseek`. The harness keeps all user data under one root. An empty or
 * whitespace-only `$VOYASEEK_HOME` is treated as unset, so a blank override never
 * resolves the home to the current working directory.
 * @param configured - explicit harness-home override, which has highest precedence.
 * @param env - environment mapping used to read `VOYASEEK_HOME`.
 * @returns the normalized absolute harness home path.
 */
export function resolveDshHome(configured?: string, env: Record<string, string | undefined> = process.env): string {
  const fromEnv = env[VOYASEEK_HOME_ENV]
  const selected = configured ?? (fromEnv !== undefined && fromEnv.trim().length > 0 ? fromEnv : defaultDshHome())
  return resolve(expandHomePath(selected))
}

/**
 * Join path segments onto the resolved Voyaseek Harness home.
 ...
 * `~/.voyaseek`, and any configured home is labelled `$VOYASEEK_HOME`.
 * @param resolvedHome - the absolute path returned by {@link resolveDshHome}.
 * @returns `~/.voyaseek` for the default home, otherwise `$VOYASEEK_HOME`.
 */
export function dshHomeDisplay(resolvedHome: string): string {
  return resolvedHome === resolve(defaultDshHome()) ? DEFAULT_VOYASEEK_HOME_DISPLAY : `$${VOYASEEK_HOME_ENV}`
}
