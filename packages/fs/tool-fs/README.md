...
`@voyaseek-ai/dsh-fs-observation-policy` is **optional**: omit it and the tools run against the bare provider (unconditional write/overwrite/edit, no observed-state). A deployment that loads these tools is expected to also load it, so the behavior is read-before-write/edit.

`read_image` registers only while a durable `ctx.attachments` service is mounted — without one the deployment cannot commit image bytes, so the tool never appears. An exact routed model that declares `image` input receives the native image block. Unknown and text-only routes use `ctx.imageFallback` automatically and receive text analysis instead; they fail only when no fallback service is mounted.
...
| `read_image` | `file_path` | Reads and persists a PNG/JPEG/WebP/GIF file. Image-capable routes receive the native block; other routes receive automatic text analysis. |
...
- **read_image** — validates the argument, extension, attachment availability, deployment media types, and route before I/O; then one `ctx.fs.stat`, a bounded `ctx.fs.readBytes`, `attachments.saveImage`, and `fs/observed`. A non-image route then calls `ctx.imageFallback` with the durable image reference and returns its text-only analysis. (1 stat.)
...
`fs/observed` fires AFTER the read/read_image/write/edit already succeeded, via a plain `ctx.emit`. A listener is contractually a synchronous, side-effect-only recorder (`@voyaseek-ai/dsh-fs-observation-policy`'s is a `WeakMap.set`); the tool does not guard the emit, so a listener that throws would surface as the tool's `isError` result — async or fallible observation does not belong on this event.
...
A successful `read_image` returns the metadata envelope followed by the native image for image-capable routes. Text-only and unknown routes instead receive text-only automatic analysis attributed to its provider and model. The attachment store still retains the durable `sha256:` image reference.
...
Failures are normalized as `Error: <message>`. Image admission reports `cannot read "<path>" as an image: automatic image analysis is unavailable` only when neither native image input nor `ctx.imageFallback` can serve the call. Other stable validation, filesystem, mismatch-repair, and guarded-mutation errors keep their existing messages and structured codes.
...
- **A concurrent model switch can select a conservative path** — `read_image` checks the latest route at execution; a later switch can make native-image versus text-analysis selection suboptimal, but both results remain consumable by the selected route admission policy.
