...
import type { CordisRunActivity } from '@voyaseek-ai/dsh-cordis-client-runner/client'
import type { SessionId } from '@voyaseek-ai/dsh-client-connection/client'
  ...
  const renderRow = (view: RowView) => {
    ...
    return (
      ...
      >
        ...
        {listed?.developmentSources !== undefined && (
          <span className={css.activeVersion} data-cordis-development-source>
            {t('panel.developmentSource', {
              path: listed.developmentSources.client ?? listed.developmentSources.host ?? '',
            })}
          </span>
        )}
        {listed?.developmentBuildError !== undefined && (
          <div className={css.rowError} role="alert" data-cordis-development-build-error>
            {t('panel.developmentBuildFailed', {
              path: listed.developmentBuildError.sourcePath ?? '',
              message: listed.developmentBuildError.message,
            })}
          </div>
        )}
