...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'Provide at least one of code.host and code.client. Host accepts JavaScript or TypeScript; Client accepts '
    ...
    parameters: {
      ...
      code: {
        ...
        required: true,
        properties: {
          host: { type: 'string', description: 'JavaScript or TypeScript function body returning the Host-half Cordis Plugin; imports, exports, and JSX are unsupported.' },
          client: { type: 'string', description: 'JavaScript, TypeScript, or TSX function body returning the browser Client-half Cordis Plugin; JSX lowers through the provided React binding and imports/exports are unsupported.' },
        },
      },
    },
    output: {
      schema: {
        ...
        properties: {
          ...
          developmentHmr: { type: 'boolean', required: true },
        },
      },
      render: (_args, value) => [{
        type: 'text',
        text: value.developmentHmr
          ? `Defined ${value.pluginId}/${value.packageId} (${value.name}); development HMR is replacing the running Client Plugin.`
          : value.developmentSources === undefined
            ? `Defined ${value.pluginId}/${value.packageId} (${value.name}); it is not running yet. `
              + 'Use cordis_run to activate this Package.'
            : `Defined ${value.pluginId}/${value.packageId} (${value.name}); this Package is not active yet. `
              + `Edit ${value.developmentSources.client ?? value.developmentSources.host ?? 'its working source'} `
              + 'for development HMR, or use cordis_run to activate it.',
      }],
    ...
    async execute(args, exec) {
      ...
      const developmentHmr = await ctx.dynamicCordisRunner.developmentHmr(
        agent,
        receipt.pluginId,
        receipt.packageId,
      )
...
function renderUnavailableReference(id: string): string {
  return [
    ...
    'It may have been removed or belong to another Session. A process restart alone does not remove persisted definitions.',
