 ...
 * `name` adds.
 * @module @voyaseek-ai/dsh-tool-cordis/inspect
 */

import type { Context, Fiber } from '@voyaseek-ai/cordis'
...
import type {} from '@voyaseek-ai/dsh-cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session\'s persistent registry.']
  }
