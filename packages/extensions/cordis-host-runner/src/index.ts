...
import { DynamicCordisPersistence } from './persistence.ts'
...
interface DevelopmentWorkingCopy {
...
export class DynamicCordisRunnerService extends TypertRemoteService {
  ...
  static Config: z<Config> = z.object({
    ...
    dshHome: z.string(),
    developmentHmr: z.boolean().default(false),
    developmentHmrPollMs: z.number().step(1).min(1).default(500),
  })

  private readonly rootCtx: Context
  private readonly registry = new DynamicCordisRegistry()
  private readonly inspectRegistry: CordisInspectRegistryService
  private readonly persistence: DynamicCordisPersistence
  private readonly starting = new Map<CordisDynamicPluginId, Promise<DynamicCordisHostHalfResult>>()
  private readonly resolved: ResolvedConfig
  private readonly developmentBaselines = new Map<CordisDynamicPluginId, { host?: string; client?: string }>()
  private readonly developmentNeedsBaseline = new Set<CordisDynamicPluginId>()
  private readonly developmentDesiredPackages = new Map<CordisDynamicPluginId, CordisDynamicPackageId>()
  private developmentPollActive = false
  private developmentDisposed = false
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.rootCtx = ctx
    this.resolved = {
      vmTimeoutMs: config.vmTimeoutMs ?? 5000,
      developmentHmr: config.developmentHmr ?? false,
      developmentHmrPollMs: config.developmentHmrPollMs ?? 500,
    }
    this.inspectRegistry = new CordisInspectRegistryService(ctx)
    this.persistence = new DynamicCordisPersistence(config.dshHome)
    const restored = this.persistence.load()
    for (const plugin of restored.plugins) {
    ...
    for (const plugin of this.registry.all()) this.captureDevelopmentBaseline(plugin)
    if (this.resolved.developmentHmr) {
      ctx.effect(() => {
        const timer = setInterval(() => { void this.pollDevelopmentSources() }, this.resolved.developmentHmrPollMs)
        timer.unref()
        return () => {
          this.developmentDisposed = true
          clearInterval(timer)
        }
      }, 'dynamic-cordis: development source watcher')
    }
  }
  ...
  define(request: DynamicCordisDefineRequest): DynamicCordisDefineReceipt {
    ...
    const host = request.code.host === undefined
      ? undefined
      : compileDynamicCordisSource(request.code.host, 'host')
    const client = request.code.client === undefined
      ? undefined
      : compileDynamicCordisSource(request.code.client, 'client')
    if (host !== undefined) precheckCode(host.code, 'code.host')
    if (client !== undefined) precheckCode(client.code, 'code.client')
    ...
    const definition: DynamicCordisDefinition = {
      ...
      name,
      purpose,
      ...host === undefined ? {} : { hostSource: host.source, hostCode: host.code },
      ...client === undefined ? {} : { clientSource: client.source, clientCode: client.code },
    }
    plugin.packages.set(packageId, definition)
    try {
      this.persist()
    } catch (error) {
      plugin.packages.delete(packageId)
      if (created) this.registry.delete(plugin.pluginId)
      throw error
    }
    const developmentSources = this.persistence.sourceDisplay(plugin.pluginId, definition)
    try {
      this.persistence.writeSources(plugin.pluginId, {
        ...host === undefined ? {} : { host: host.source },
        ...client === undefined ? {} : { client: client.source },
      })
      this.captureDevelopmentBaseline(plugin)
      delete plugin.developmentBuildError
    } catch (error) {
      // The manifest already committed this Package. A working-copy failure is
      // diagnostic only; the next successful read captures its actual bytes as
      // a baseline before the watcher may publish a later user edit.
      this.developmentNeedsBaseline.add(plugin.pluginId)
      this.recordDevelopmentBuildError(plugin, error)
    }
    return {
      ...
      ...this.resolved.developmentHmr ? { developmentSources } : {},
    }
  }
   ...
   */
  async developmentHmr(
    agent: Agent,
    pluginId: CordisDynamicPluginId,
    packageId: CordisDynamicPackageId,
  ): Promise<boolean> {
    if (!this.resolved.developmentHmr) return false
    const plugin = this.owned(agent, pluginId)
    const definition = plugin?.packages.get(packageId)
    if (plugin === undefined
      || definition?.clientCode === undefined
      || plugin.run === undefined
      || plugin.currentPackageId === undefined
      || plugin.run.packageId !== plugin.currentPackageId
      || !plugin.clientVersionUpdatesApproved) return false
    const response = await this.run(agent, pluginId, packageId, 'update')
    return response.ok && response.status === 'starting'
  }

  /** Read or seed the stable working copy and make its current bytes the watch baseline. */
  private captureDevelopmentBaseline(plugin: DynamicCordisPlugin): DevelopmentWorkingCopy {
    const definition = [...plugin.packages.values()].at(-1)
    if (definition === undefined) throw new Error(`dynamic plugin "${plugin.pluginId}" has no package`)
    const working = this.persistence.readSources(plugin.pluginId, definition)
    this.developmentBaselines.set(plugin.pluginId, { ...working.source })
    return working
  }

  /** Poll every Plugin working copy once without allowing overlapping rebuilds. */
  private async pollDevelopmentSources(): Promise<void> {
    if (this.developmentPollActive || this.developmentDisposed) return
    this.developmentPollActive = true
    try {
      for (const plugin of this.registry.all()) {
        if (this.developmentDisposed) return
        await this.convergeDevelopmentHmr(plugin)
        const definition = [...plugin.packages.values()].at(-1)
        if (definition === undefined) continue
        let working: DevelopmentWorkingCopy
        try {
          working = this.persistence.readSources(plugin.pluginId, definition)
        } catch (error) {
          this.recordDevelopmentBuildError(plugin, error)
          continue
        }
        if (this.developmentNeedsBaseline.delete(plugin.pluginId)) {
          this.developmentBaselines.set(plugin.pluginId, { ...working.source })
          if (sameDevelopmentSource(working.source, {
        ...
        const baseline = this.developmentBaselines.get(plugin.pluginId)
        if (sameDevelopmentSource(baseline, working.source)) continue
        // A broken edit is reported once and remains the baseline until the
        // author changes the file again; the last compiled revision stays live.
        this.developmentBaselines.set(plugin.pluginId, { ...working.source })
        await this.publishDevelopmentRevision(plugin, definition, working)
      }
    } finally {
      this.developmentPollActive = false
    }
  }

  /** Build and durably append one working-copy revision, then request a safe live replacement. */
  private async publishDevelopmentRevision(
    plugin: DynamicCordisPlugin,
    previous: DynamicCordisDefinition,
    working: DevelopmentWorkingCopy,
  ): Promise<void> {
    let host: ReturnType<typeof compileDynamicCordisSource> | undefined
    let client: ReturnType<typeof compileDynamicCordisSource> | undefined
    try {
      if (working.source.host !== undefined) {
        try {
          host = compileDynamicCordisSource(working.source.host, 'host')
          precheckCode(host.code, 'code.host')
        } catch (error) {
          this.recordDevelopmentBuildError(plugin, error, working.display.host)
          return
        }
      }
      if (working.source.client !== undefined) {
        try {
          client = compileDynamicCordisSource(working.source.client, 'client')
          precheckCode(client.code, 'code.client')
        } catch (error) {
          this.recordDevelopmentBuildError(plugin, error, working.display.client)
          return
        }
      }
      const packageId = CordisDynamicPackageId(this.registry.mintPackageId())
      ...
      if (agent !== undefined
        && plugin.run !== undefined
        && plugin.currentPackageId !== undefined
        && plugin.clientVersionUpdatesApproved) {
        this.developmentDesiredPackages.set(plugin.pluginId, packageId)
        await this.convergeDevelopmentHmr(plugin)
      }
    } catch (error) {
      this.recordDevelopmentBuildError(plugin, error)
  ...
  /** Retain one dev-only failure for inventory surfaces without changing durable state. */
  private recordDevelopmentBuildError(
    plugin: DynamicCordisPlugin,
    error: unknown,
    sourcePath?: string,
  ): void {
    const details = errorDetails(error)
    plugin.developmentBuildError = {
      message: details.message,
      ...sourcePath === undefined ? {} : { sourcePath },
    }
    this.ctx.logger.warn(`dynamic Cordis development build of "${plugin.pluginId}" failed: ${details.message}`)
  }

  /** Move a queued development revision onto the existing authorized run path when the prior transition settles. */
  private async convergeDevelopmentHmr(plugin: DynamicCordisPlugin): Promise<void> {
    const desired = this.developmentDesiredPackages.get(plugin.pluginId)
    if (desired === undefined || this.developmentDisposed) return
    if (plugin.currentPackageId === desired && plugin.run?.packageId === desired) {
      this.developmentDesiredPackages.delete(plugin.pluginId)
      return
    }
    const latest = plugin.latestRun
    if (latest?.packageId === desired
      && ['failed', 'rejected', 'cancelled', 'stopped'].includes(latest.status)) {
      this.developmentDesiredPackages.delete(plugin.pluginId)
      return
    }
    if (this.starting.has(plugin.pluginId)
      || this.registry.pendingRequestFor(plugin.pluginId) !== undefined
      || (plugin.run !== undefined && plugin.run.packageId !== plugin.currentPackageId)) return
    ...
    await this.developmentHmr(agent, plugin.pluginId, desired)
  }
  ...
  async undefine(agent: Agent, pluginId: CordisDynamicPluginId): Promise<DynamicCordisUndefineReceipt> {
    ...
    this.persist(this.registry.all().filter(candidate => candidate !== plugin))
    this.registry.delete(pluginId)
    this.developmentBaselines.delete(pluginId)
    this.developmentNeedsBaseline.delete(pluginId)
    this.developmentDesiredPackages.delete(pluginId)
  ...
  inventory(): DynamicCordisInventoryRow[] {
    return this.registry.all().map((plugin) => {
      const latest = [...plugin.packages.values()].at(-1)
      const developmentSources = latest === undefined
        ? undefined
        : this.persistence.sourceDisplay(plugin.pluginId, latest)
      return {
        pluginId: plugin.pluginId,
        ...
        ...!this.resolved.developmentHmr || developmentSources === undefined ? {} : { developmentSources },
        ...plugin.developmentBuildError === undefined ? {} : {
          developmentBuildError: { ...plugin.developmentBuildError },
        },
      }
    })
  }
  ...
  private async startFresh(
    ...
    if (definition.clientCode === undefined) {
      try {
        await this.commitActivation(plugin, run)
      } catch (error) {
  ...
  private async settleActivation(
    ...
    try {
      ...
      const failure = errorDetails(error)
      if (attempt !== undefined && attempt.pluginRunId === run.pluginRunId) {
        this.failAttempt(plugin, attempt, 'client-apply', failure)
      }
  ...
  private async commitActivation(plugin: DynamicCordisPlugin, run: DynamicCordisRun): Promise<void> {
    const previousCurrent = plugin.currentPackageId
    const previousNext = plugin.nextPackageId
    plugin.currentPackageId = run.packageId
    delete plugin.nextPackageId
    try {
      this.persist()
    } catch (error) {
      if (previousCurrent === undefined) delete plugin.currentPackageId
      else plugin.currentPackageId = previousCurrent
      if (previousNext === undefined) delete plugin.nextPackageId
      else plugin.nextPackageId = previousNext
      throw error
    }
  ...
  /** Publish durable definitions and stable identity counters. */
  private persist(plugins: DynamicCordisPlugin[] = this.registry.all()): void {
    this.persistence.save({ plugins, ...this.registry.identities() })
  }
}
...
function missingPluginMessage(id: CordisDynamicPluginId): string {
  return `no dynamic plugin "${id}" in the persistent registry — it may have been removed`
}

/** Whether two optional Host/Client working copies contain identical bytes. */
function sameDevelopmentSource(
  left: { host?: string; client?: string } | undefined,
  right: { host?: string; client?: string },
): boolean {
  return left?.host === right.host && left?.client === right.client
}
