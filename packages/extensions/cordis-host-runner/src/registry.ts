...
export interface DynamicCordisDefinition {
  ...
  /** User-facing purpose. */
  purpose: string
  /** Original Host TypeScript/JavaScript source retained for inspection. */
  hostSource?: string
  /** Original Client TSX/JavaScript source retained for inspection. */
  clientSource?: string
  /** Built Host JavaScript function body consumed by the VM evaluator. */
  hostCode?: string
  /** Built Client JavaScript function body consumed by the browser evaluator. */
  clientCode?: string
}

/** Last development build failure for one editable working copy. */
...
/** Durable identities and definitions restored before the service is exposed. */
export interface DynamicCordisRegistrySnapshot {
  /** Stable Plugins and immutable Packages in creation order. */
  plugins: DynamicCordisPlugin[]
  /** First unused global Plugin suffix. */
  nextPlugin: number
  /** First unused immutable Package suffix. */
  nextPackage: number
}
...
export interface DynamicCordisPlugin {
  ...
  /** Process-local development build error; never persisted as lifecycle state. */
  developmentBuildError?: DynamicCordisDevelopmentBuildError
}
...
export interface DynamicCordisDefineReceipt {
  ...
  /** Stable editable working-copy paths, exposed only while development HMR is enabled. */
  developmentSources?: { host?: string; client?: string }
}
...
export class DynamicCordisRegistry {
  ...
  /**
   * Restore durable definitions into a fresh process. Physical runs, pending
   * approvals, and grants are deliberately absent from the snapshot.
   * @param snapshot - validated persistent definitions and identity counters.
   */
  restore(snapshot: DynamicCordisRegistrySnapshot): void {
    if (this.plugins.size !== 0 || this.pendingRequests.size !== 0) {
      throw new Error('dynamic Cordis registry can only restore into an empty process')
    }
    for (const plugin of snapshot.plugins) {
      if (this.plugins.has(plugin.pluginId)) throw new Error(`duplicate dynamic plugin "${plugin.pluginId}"`)
      this.plugins.set(plugin.pluginId, plugin)
    }
    this.nextPlugin = snapshot.nextPlugin
    this.nextPackage = snapshot.nextPackage
  }

  /**
   * Capture the durable Plugin/Package identity counters. Run and approval IDs
   * are process-local and intentionally restart from one.
   * @returns the first unused stable identity suffixes.
   */
  identities(): Pick<DynamicCordisRegistrySnapshot, 'nextPlugin' | 'nextPackage'> {
    return { nextPlugin: this.nextPlugin, nextPackage: this.nextPackage }
  }
