...
import ToolRegistry from '@voyaseek-ai/dsh-tools'
import type { ToolDefinition, ToolExecutionResult } from '@voyaseek-ai/dsh-tools'
import type { CordisDynamicPluginId } from '../src/types.ts'
import type { DynamicCordisClientSource } from '../src/types.ts'
...
interface Gateway {
  ...
  /** Whether the fake user's approval covers later Plugin versions. */
  approveFutureVersions?: boolean
  /** Optional real Client loader used by cross-runtime integration specs. */
  loadClient?: (source: DynamicCordisClientSource) => Promise<
    | { ok: true; waitingFor?: readonly string[] }
 ...
 * @param liveAgents - optional Agent-registry rows used by the development watcher.
 * @returns the context, the runner service, and the gateway recorder.
 */
export async function setup(config?: Config, liveAgents: readonly Agent[] = []): Promise<Harness> {
  ...
  if (liveAgents.length > 0) {
    ctx.provide('agents', {
      get: (id: SessionId) => liveAgents.find(agent => agent.id === id),
    } as never)
  }
  ...
  ctx.on('cordis/request-run', (request) => {
    ...
    gateway.answering = Promise.resolve().then(async (): Promise<void> => {
      ...
      const half = await runner.runHostHalf(
        AGENT_A,
        pluginId,
        packageId,
        mode,
        requestId,
        gateway.approveFutureVersions ?? false,
      )
      if (!half.ok) {
      ...
      const loaded = await gateway.loadClient?.(source)
      ...
      await runner.resolveRequestRun(requestId, {
        ok: true,
        pluginRunId: source.pluginRunId,
        ...loaded?.waitingFor === undefined && gateway.clientWaitingFor === undefined
          ? {}
          : { waitingFor: loaded?.waitingFor ?? gateway.clientWaitingFor },
      })
    })
  })
  ...
  const dshHome = config?.dshHome ?? mkdtempSync(join(tmpdir(), 'dsh-cordis-runner-'))
  if (config?.dshHome === undefined) temporaryHomes.add(dshHome)
  await ctx.plugin(DynamicCordisRunnerService, { ...config, dshHome })
