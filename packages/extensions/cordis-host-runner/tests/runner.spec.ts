...
describe('dynamic runner definitions', () => {
  ...
  it('builds TypeScript before recording the immutable Package', async () => {
    const { runner } = await setup()

    const receipt = define(runner, {
      sessionId: AGENT_A.id,
      name: 'typed',
      purpose: 'p',
      client: 'return { type: \'text\' as const }',
    })
    expect(runner.inspectPackage(AGENT_A, receipt.pluginId, receipt.packageId).code.client)
      .toBe('return { type: \'text\' as const }')
    expect(running(runner, AGENT_A)).toEqual([{ id: 'dyn-1', running: false }])
  })
...
describe('dynamic runner dispatch', () => {
  ...
  it('hands the browser half\'s source only to the owning session, and only while it runs', async () => {
    ...
    expect(runner.getClientCode(AGENT_A, pluginId, started.pluginRunId)).toMatchObject({
      code: expect.stringContaining('return () =>'), name: 'ui', pluginId, packageId,
      pluginRunId: started.pluginRunId,
    })
    expect(runner.inspectPackage(AGENT_A, pluginId, packageId).code.client).toBe(CLIENT_CODE)
...
describe('dynamic runner teardown', () => {
  ...
  it('answers a missing definition from the persistent registry', async () => {
    const { runner } = await setup()
    ...
    expect((receipt as { message: string }).message).toContain('persistent registry')
  })
