...
describe('build failures teach the fix', () => {
  it('accepts TypeScript syntax and stores the inspectable source at define time', async () => {
    const harness = await setup()
    const receipt = harness.runner.define({
    ...
    expect(harness.runner.inspectPackage(AGENT_A, receipt.pluginId, receipt.packageId).code.host)
      .toContain('as const')
    expect(running(harness.runner, AGENT_A)).toEqual([{ id: 'probe-1', running: false }])
  })
  ...
  it('surfaces the offending line + caret and the bracket-balance hint on a syntax error', async () => {
    ...
    expect(message).toContain('code.host TypeScript compile failed')
