...
| [`tool-plugin-discovery/`](tool-plugin-discovery/README.md) | Curated DSH plugin discovery leads | registers on `ctx.tools` |
| [`tool-modelscope/`](tool-modelscope/README.md) | Read-only official ModelScope Hub model search | registers on `ctx.tools` |
