# @voyaseek-ai/dsh-cordis-client-runner

English | [中文](README.zh.md)

Browser half of dynamic dual-half plugin packages. The host-side runner restores durable definitions and verified JavaScript artifacts, then asks open pages, over a `cordis/request-run` event, whether to run one; this package answers that request, turns the exact built artifact into a live browser plugin, and turns a `cordis/dynamic-retract` event back into a clean page.
...
2. **Closure evaluation** — the browser half's built JavaScript runs as an async function body whose parameters are its symbol surface (`React`, `console`, `styles`, `host`, plus teaching traps shadowing `setTimeout`/`fetch`/`require`). Authors may define JavaScript, TypeScript, or TSX; the Host lowers TSX to this existing `React` binding before publication. Imports and exports remain unsupported because the closure has no module resolver.
...
Loads converge by Plugin Run ID against live state: loading an activation this page already runs answers from live state without reloading (so a replayed run does not look unanswered), a newer activation first removes the Loader entry and drains its Fiber disposers, then mounts the new artifact, and the same Package after a retract loads afresh. Operations serialize per Plugin, so overlapping development rebuilds cannot interleave teardown and mount.

Nothing loads merely because the Host process or a page restarted. Durable Package definitions remain stopped until someone answers a run request or asks for one here; development HMR uses that same run path and only bypasses a new prompt after the user already approved future versions of the live Plugin.
