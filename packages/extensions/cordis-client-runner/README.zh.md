...
动态双半插件包的浏览器半。host 侧 runner 恢复持久定义与校验过的 JavaScript artifact，并经一条 `cordis/request-run` 事件向打开的页面发问「要不要运行它」；本包回答这个请求、把精确的编译产物变成活的浏览器插件，并把 `cordis/dynamic-retract` 事件变回干净的页面。
...
2. **闭包求值** —— 浏览器半编译后的 JavaScript 作为一个 async 函数体运行，其参数即符号面（`React`、`console`、`styles`、`host`，外加遮蔽 `setTimeout`/`fetch`/`require` 的教学陷阱）。作者可以 define JavaScript、TypeScript 或 TSX；Host 会在发布前把 TSX 降级到这里已有的 `React` 绑定。闭包没有 module resolver，因此仍不支持 import／export。
...
装载按 Plugin Run ID 对 live 态收敛：装载这一页已在运行的 activation 会**直接从 live 态回答**而不重装（所以被重播的 run 不会看起来没人回答），更新的 activation 会先移除 Loader entry 并排空其 Fiber disposer，再挂载新 artifact；同一个 Package 在 retract 之后再装则重新装载。同一个 Plugin 的操作串行执行，因此重叠的开发构建不会交错 teardown 与 mount。

Host 进程或页面重启本身都不会装载任何东西。持久 Package 定义保持 stopped，直到有人回答一次 run 请求、或在这一页主动要求运行；开发态 HMR 使用同一条 run 路径，并且只会在用户已经允许存活 Plugin 的后续版本后免去新的提示。
