...
工具可见时，模型会看到生成的 [`modelscope_search` schema](../../../docs/tool-catalog.md#voyaseek-aidsh-tool-modelscope)；模型调用后还会看到一份有界目录结果。本包不添加常驻提示词段。

#### Token 影响

工具可见时产生固定 schema 成本；每次调用还会产生随数据变化的结果文本。

#### KV Cache 影响

工具定义在配置与可见性不变时保持前缀稳定。每次调用和结果会扩展当前请求后缀。
