...
import type { Session, TodoItem } from '@voyaseek-ai/dsh-session'
...
describe('todos projection provider', () => {
  ...
  it('settles active and unstarted work when the turn ends in error', async () => {
    const bench = await harness(true)
    const session = bench.session
    seedMessage(session)
    ...
    session.append('turn/end', {
      turn: 1,
      reason: { kind: 'error', error: { code: 'RATE_LIMIT', message: 'raw provider body' } },
    })

    const todos = (await bench.tailProjections())?.values.todos
    expect(todos).toEqual([
      { content: 'already done', status: 'completed' },
      { content: 'active work', status: 'failed' },
      { content: 'not started', status: 'blocked' },
    ])
    expect(JSON.stringify(todos)).not.toMatch(/in_progress|pending/)
  })
