 ...
 * @module @voyaseek-ai/dsh-tool-todo
 */
...
import type { TodoItem } from '@voyaseek-ai/dsh-session'
import type { TodoProjectionItem } from './types.ts'
...
const todosProjectionSchema: ZodType<TodoProjectionItem[] | null> = zod.union([
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.inject(['sessionProjections'], (projectionCtx) => {
    projectionCtx.sessionProjections.register<'todos', TodoProjectionItem[] | null>({
      key: 'todos',
      ...
      apply: (state, event) => {
        ...
        if (event.type === 'turn/end' && event.data.reason.kind === 'error' && state !== null) {
          let changed = false
          const settled = state.map((todo): TodoProjectionItem => {
            if (todo.status === 'completed' || todo.status === 'failed' || todo.status === 'blocked') return todo
            changed = true
            return {
              content: todo.content,
              status: todo.status === 'in_progress' ? 'failed' : 'blocked',
            }
          })
          return changed ? settled : state
        }
        return state
      },
      view: state => state,
