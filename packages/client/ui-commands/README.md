# @voyaseek-ai/dsh-client-ui-commands
...
Menu queries fuzzy-match ordered, case-insensitive subsequences of stable command names. Shipped Host commands receive locale-aware labels and descriptions when candidates are synthesized; changing the active locale affects the next candidate pass without refetching the Host directory, while unknown commands retain their Host name and description. The localized label is display-only: picks, space/Enter matching, and execution continue to use the stable command name. Prefixes rank first; separator boundaries, adjacent characters, and shorter gaps rank the remaining matches, with directory and contribution order breaking ties. Rationale: [Web slash-command fuzzy discovery](../../../.agents/notes/implemented/feature/2026-08-04-web-slash-command-fuzzy-discovery.md).
