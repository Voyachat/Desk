...
菜单查询会按顺序且不区分大小写地模糊匹配稳定命令名的子序列。生成 candidate 时，内置 Host 命令会获得跟随当前 locale 的标签与说明；切换语言后，下一次 candidate pass 直接使用新语言，无需重新拉取 Host 目录，未知命令则保留 Host 名称与说明。本地化标签只用于展示；pick、space/Enter 匹配与执行仍使用稳定命令名。前缀排名最高；其余匹配项按分隔符边界优先、相邻字符优先、间隔越短越优先的规则排序，若仍同分，则以目录顺序和贡献项顺序打破平局。原理：[Web 斜杠命令模糊发现](../../../.agents/notes/implemented/feature/2026-08-04-web-slash-command-fuzzy-discovery.md)。
