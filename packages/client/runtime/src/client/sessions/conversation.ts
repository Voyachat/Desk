...
import type { ContentBlock } from '@voyaseek-ai/dsh-llm/types'
import type { ImageAttachmentRef } from '@voyaseek-ai/dsh-attachment'
import type { LlmRetryEventData } from '@voyaseek-ai/dsh-llm-retry/types'
...
import type { FailureDisplayCategory } from './failure-display.ts'
import type {
...
export interface TurnErrorNode {
  ...
  turn: number
  step: number
  /** Stable user-action class; presentation localizes all primary copy from it. */
  category: FailureDisplayCategory
  /** Bounded detail assembled from code/status/retry/request-id fields only. */
  diagnostic: string
  /** Provider-requested retry delay when present and valid. */
  retryAfterMs?: number
}
