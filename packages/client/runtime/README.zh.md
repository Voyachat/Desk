...
Host 所属的 LLM（大语言模型）retry invariant 会在持久追加边界验证按提供方路由的 `llm/retry` 与 `llm/retry-started` 记录，包括标识、顺序、计时器、整数、状态、提供方延迟和非空诊断字段约定。客户端的 Retry、Assistant 与 Turn Error Definition 把这些记录和 Assistant、Turn／Step 事件一起折叠：失败步骤的流式输出片段会被移除，并在 retry 事件的序列位置插入一条持久重试提示。该提示在匹配的 started 记录到达前为 `scheduled`；如果所属 Step 或 Turn 先关闭，则标记为 `cancelled`，started 记录到达后则标记为 `started`。normal mode 提示携带其有限上限；always mode 提示保持显式无界。共享失败展示投影只根据稳定错误码和 HTTP 状态分类重试与终态失败，保留有效的提供方延迟，并且只用错误码、状态、延迟和请求 id 构建有界诊断；提供方消息文本不会被复制到展示事实中。因此，没有重试的终态 `turn/end` 错误只会投影出携带类别、安全诊断和可选重试延迟的 `turn-error` 节点。进入重试的失败只保留该次尝试的重试提示，其展示也使用同一安全投影。窗口重建与历史回放使用同一组 Definition，因此刷新既不会让已丢弃的分片重新出现，也不会丢失终态失败反馈。可见但尚未定稿的输出会在终态错误旁冻结为中断的 Assistant 节点。
...
`ISessions.fork({sessionId, atSeq?, increaseTitle?, agentRuntime?})` 只在子会话摘要已能在本地寻址后才完成；该摘要携带源会话的谱系和 cwd，且 `blank: false`，由调用方决定是否打开。省略 `agentRuntime` 时继承源模式；显式 id 选择该模式，`''` 则选择本机循环。Client 会把该 Runtime 选择发送给 Host，并立即投影到子会话行。`increaseTitle: true` 会在 client 端根据源会话的持久化标题重命名子会话：尾部 `(N)` 或 `（N）` 递增并保留括号样式，其余标题追加 ` (1)`；源会话没有持久化标题时跳过改名，改名失败时拒绝 promise 但保留已创建的子会话。标题选项不会进入 Host fork 请求。即使响应为 `workspace-attach-failed`，其中仍会标识 Host 已发布的子会话，因此 `SessionManager` 会先将这一部分成功对账，再让 `SessionForkError` 到达调用方，避免重试创建重复的子会话。
