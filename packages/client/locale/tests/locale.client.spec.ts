...
import { stubSettingsScope, type StubSettingsScope } from '@voyaseek-ai/dsh-client-test-runtime'
import type { LocaleSettings, LocaleSnapshot } from '@voyaseek-ai/dsh-client-locale/client'
...
describe('LocaleRuntime', () => {
  ...
  it('an absent writable Host preference adopts and persists the browser-derived locale', () => {
    const host = stubSettingsScope<LocaleSettings>()
    const { svc } = make(host)
    host.publish({ status: 'ready', value: { preference: 'en' }, revision: 1, writable: true })
    expect(svc.getLocale().active).toBe('en')
  ...
  it('does not try to persist a provisional locale through a read-only or remote scope', () => {
    const readOnly = stubSettingsScope<LocaleSettings>()
    make(readOnly)
    readOnly.publish({ status: 'ready', value: {}, revision: 1, writable: false })
    expect(readOnly.set).not.toHaveBeenCalled()

    const remote = stubSettingsScope<LocaleSettings>()
    make(remote)
    remote.publish({ status: 'ready', value: {}, revision: 1, writable: true, mode: 'memory' })
