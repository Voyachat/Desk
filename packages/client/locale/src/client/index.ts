...
import type { ClientContext, SettingsScope } from '@voyaseek-ai/dsh-client-runtime/client'
...
import {
  FALLBACK_LOCALE, LOCALE_PREFERENCE_FIELD, LOCALE_SETTINGS_NAMESPACE, type LocaleId, type LocaleSettings,
...
export type { Translate, TranslateNS } from '@voyaseek-ai/dsh-client-ui-slots'
...
export { FALLBACK_LOCALE } from '../locale-settings.ts'
...
export class LocaleRuntime {
  ...
  /** Raw Host revision whose absent preference already triggered one provisional write. */
  private provisionalWriteRevision: number | undefined
  ...
  private adopt(host: SettingsScope<LocaleSettings>): void {
    ...
    if (section.preference === undefined
      ...
      && this.provisionalWriteRevision !== snapshot.revision) {
      const revision = snapshot.revision
      this.provisionalWriteRevision = revision
      void host.set(LOCALE_PREFERENCE_FIELD, this.provisional).finally(() => {
        if (this.provisionalWriteRevision === revision) this.provisionalWriteRevision = undefined
      })
    } else if (section.preference !== undefined) {
      this.provisionalWriteRevision = undefined
    }
