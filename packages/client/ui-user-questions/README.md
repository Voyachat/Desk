# @voyaseek-ai/dsh-client-ui-user-questions
...
Selection state is local to a component keyed by the request rpcId. The card can collapse into a one-line pending-question bar without cancelling the wait; expanding the same request restores its current page, option selections, and custom text. A replay with the same id preserves a still-mounted draft, while `question/resolved` from the host removes the composer. The host remains authoritative: successful HTTP delivery does not remove pending state locally.
