...
选择状态只存在于以请求 rpcId 为 key 的组件本地。卡片可收起为单行待处理问题条，且不会取消等待；再次展开同一请求时会恢复当前页、选项选择和自定义文本。使用相同 id 回放时，只要组件仍挂载，就会保留草稿；主机发出的 `question/resolved` 则会移除编辑器。主机仍具有最终决定权：HTTP 交付成功不会在本地移除待处理状态。
