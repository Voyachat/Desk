...
describe('createFixtureApi', () => {
  ...
  it('respond answers the resident approval once: routing, validation, resolved broadcast, then not-pending', async () => {
    ...
    expect(requested.frame.rememberable).toBe(true)
    ...
    expect(await api.respond({ type: 'client-response', rpcId: RpcId(requested.rpcId), result: { ok: true, value: { sessionId: sid('fx-alpha'), approvalId, outcome: 'allowed-and-remembered' } } }))
      .toEqual({ accepted: true })
    await vi.waitFor(() => {
      expect(seen.some(s => s.frame.type === 'approval/resolved' && s.frame.outcome === 'allowed-and-remembered')).toBe(true)
    })
