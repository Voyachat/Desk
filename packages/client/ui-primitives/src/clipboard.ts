 ...
 * falling back to `execCommand('copy')` when the async path is unavailable
 * or rejected by the host.
...
export async function writeClipboard(text: string): Promise<boolean> {
  ...
  if (navigator.clipboard?.writeText) {
    try {
      ...
      // Electron and iframe policies may reject the async API while allowing
      // the user-gesture-scoped DOM copy path below.
    }
  }
  // jsdom, Electron permission policies, and older hosts: best-effort
  // execCommand path when present.
