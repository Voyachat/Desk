...
import { shallowEqual, type SessionId, type SessionListState } from '@voyaseek-ai/dsh-client-runtime/client'
...
export interface WorkflowRunInjected {
  ...
  /** Execute the Host-owned explicit from-start retry command. */
...
export function workflowDotState(status: WorkflowRunStatus): StateDotState {
  switch (status) {
...
function phaseRequiresExpansion(phase: WorkflowRunPhaseData): boolean {
  return phase.members.some(member => member.status !== 'succeeded')
}
...
function phaseStatusSummary(members: readonly WorkflowRunMemberData[], t: WorkflowRunPanelProps['t']): string {
  ...
  const active = (['pending', 'running', 'failed', 'cancelled', 'interrupted'] as const)
    .filter(status => count(status) > 0)
  if (active.length === 0) return statusCount('succeeded', count('succeeded'), t)
  const visible = active.includes('interrupted') && count('succeeded') > 0
    ? ['succeeded' as const, ...active]
    : active
      ...
      collapsedContent={(
        <>
          ...
          <span className={css.statusTail} data-status={status}>
            <StateDot state={workflowDotState(status)} />
            <span>{t(WORKFLOW_STATUS_KEYS[status])}</span>
          </span>
        </>
      )}
    >
...
function MemberRow({ member, navigable, openSession, t }: {
  ...
  const content = (
    <>
      <span className={css.dotSlot}><StateDot state={workflowDotState(member.status)} /></span>
      ...
      <span className={css.memberStatus} data-member-status-text>{t(WORKFLOW_STATUS_KEYS[member.status])}</span>
    </>
  )
  if (!navigable) {
...
export function WorkflowRunPanel({
  node, sessionId, useSessions, openSession, retryRun, inspectCall, t,
}: WorkflowRunPanelProps) {
  const [retryState, setRetryState] = useState<'idle' | 'confirm' | 'running' | 'failed'>('idle')
  const totalMembers = node.data.phases.reduce((count, phase) => count + phase.members.length, 0)
  const requiresExpansion = node.data.status !== 'succeeded'
  ...
  const retryLabel = retryState === 'confirm'
    ? t('action.confirm')
    : retryState === 'running'
      ? t('action.running')
      : node.data.status === 'interrupted' ? t('action.resume') : t('action.retry')
  const onRetry = (): void => {
    if (retryState === 'running') return
    if (retryState !== 'confirm') {
      setRetryState('confirm')
      return
    }
    setRetryState('running')
    void retryRun(node.id).then(
      () => { setRetryState('idle') },
    ...
    <section className={css.root} data-workflow-run data-run-status={node.data.status}>
      ...
      >
        <div className={css.actions}>
          <button type="button" className={css.actionButton} onClick={() => { inspectCall(node.data.callId) }}>
            {t('action.inspect')}
          </button>
          {node.data.resumable
            ? (
              <>
                <span className={css.resumable}>{t('recovery.resumable')}</span>
                <button
                  type="button"
                ...
                {retryState === 'failed' ? <span className={css.actionError}>{t('action.retryFailed')}</span> : null}
              </>
            )
            : null}
        </div>
