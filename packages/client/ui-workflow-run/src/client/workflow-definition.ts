...
export interface WorkflowRunChatData {
  ...
  /** Original tool call used by inspect and explicit from-start retry. */
  readonly callId: ToolWorkflowRunStartData['callId']
  readonly status: WorkflowRunStatus
  /** True only when a terminal run may be explicitly restarted from its logged source call. */
...
function statusFromStopReason(stopReason: ToolWorkflowRunStopReason): WorkflowRunStatus {
  switch (stopReason) {
    case 'completed': return 'succeeded'
    case 'cancelled': return 'cancelled'
    case 'error': return 'failed'
    case 'interrupted': return 'interrupted'
    /* v8 ignore next -- ToolWorkflowRunStopReason is closed and every variant is handled above. */
    default: return stopReason satisfies never
  }
}

function statusFromOutcome(outcome: ToolWorkflowAgentOutcome): WorkflowRunStatus {
  switch (outcome) {
    case 'completed': return 'succeeded'
    case 'cancelled': return 'cancelled'
    case 'failed': return 'failed'
    case 'interrupted': return 'interrupted'
    /* v8 ignore next -- ToolWorkflowAgentOutcome is closed and every variant is handled above. */
...
function projectWorkflow(
  ...
  return {
    ...
    status: state.stopReason === undefined
      ? interrupted ? 'interrupted' : state.members.length === 0 ? 'pending' : 'running'
      : statusFromStopReason(state.stopReason),
    resumable: state.stopReason === 'interrupted' || state.stopReason === 'error',
...
export const workflowRunDefinition: ConversationNodeDefinition<WorkflowState> = {
  ...
  start: (_context, match) => {
    ...
    return { name: match.event.data.name, callId: match.event.data.callId, members: [] }
  },
