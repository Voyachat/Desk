...
import type {
  FailureDisplayCategory, ModelRetryNode, TurnErrorNode, UserMessageNode,
} from '@deepseek-ai/dsh-client-runtime/client'
import { projectFailureDisplay } from '@deepseek-ai/dsh-client-runtime/client'
...
function ModelRetryItem({ node, active, t }: {
  ...
  const failure = projectFailureDisplay(node.failure)
  const failureCopy = modelFailureCopy(failure.category, failure.retryAfterMs, t)
    ...
    <details className={css.retryRow} data-active={active || undefined}>
      ...
      <div className={css.retryDetails}>
        ...
        <div>
          ...
          {failureCopy.title} — {failureCopy.hint}
        </div>
        <div>
          <span className={css.retryDetailLabel}>{t('message.turnError.diagnostics')}: </span>
          <code className={css.retryDiagnostic}>{failure.diagnostic}</code>
        </div>
      </div>
    </details>
  )
...
function modelFailureCopy(
  category: FailureDisplayCategory,
  ...
  switch (category) {
    case 'quota': return {
      title: t('message.turnError.quota.title'),
      hint: t('message.turnError.quota.hint'),
    }
    case 'rate-limit': return {
      title: t('message.turnError.rateLimit.title'),
      hint: retryAfterMs === undefined
        ? t('message.turnError.rateLimit.hint')
        : t('message.turnError.rateLimit.retryAfter', { seconds: retrySeconds(retryAfterMs) }),
    }
    case 'auth': return {
      title: t('message.turnError.auth.title'),
      hint: t('message.turnError.auth.hint'),
    }
    case 'network': return {
      title: t('message.turnError.network.title'),
      hint: t('message.turnError.network.hint'),
    }
    case 'timeout': return {
      title: t('message.turnError.timeout.title'),
      hint: t('message.turnError.timeout.hint'),
    }
    ...
    case 'context-window': return {
      ...
      hint: t('message.turnError.contextWindow.hint'),
    }
    case 'invalid-request': return {
      title: t('message.turnError.invalidRequest.title'),
      hint: t('message.turnError.invalidRequest.hint'),
    }
    case 'configuration': return {
      title: t('message.turnError.configuration.title'),
      hint: t('message.turnError.configuration.hint'),
    }
    case 'unknown': return {
      title: t('message.turnError.unknown.title'),
      hint: t('message.turnError.unknown.hint'),
    }
  }
}
...
function TurnErrorItem({ node, t }: {
  ...
  const copy = modelFailureCopy(node.category, node.retryAfterMs, t)
  return (
    <div className={css.turnErrorRow} data-turn-error-category={node.category}>
      <StateDot state="error" className={css.turnErrorDot} />
      <div className={css.turnErrorCopy}>
        <div role="status">
          <span className={css.turnErrorTitle}>{copy.title}</span>
          <span className={css.turnErrorMessage}>{copy.hint}</span>
        </div>
        <details className={css.turnErrorDiagnostic}>
          <summary>{t('message.turnError.diagnostics')}</summary>
          <code className={css.turnErrorCode}>{node.diagnostic}</code>
        </details>
      </div>
    </div>
  )
