...
interface TurnErrorState {
  ...
  readonly failure?: {
    ...
    readonly display: ReturnType<typeof projectFailureDisplay>
  }
}
...
function failureFrom(match: ConversationMatch): TurnErrorState['failure'] | undefined {
  ...
  return {
    ...
    display: projectFailureDisplay(failure),
  }
}
...
export const turnErrorDefinition: ConversationNodeDefinition<TurnErrorState> = {
  ...
  buildViewNode: (context) => {
    ...
    const node: TurnErrorNode = {
      ...
      category: failure.display.category,
      diagnostic: failure.display.diagnostic,
      ...failure.display.retryAfterMs === undefined
        ? {}
        : { retryAfterMs: failure.display.retryAfterMs },
    }
