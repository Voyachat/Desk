...
import { IconChecklistOutline14, IconChevronDownOutline14, IconChevronUpOutline14 } from '@voyaseek-ai/dsh-client-ui-primitives'
import { NS } from '../locales.ts'
import css from './TodoPanel.module.css'

export interface TodoPanelProps {
  /** The session's current plan (empty renders nothing) — selected by the dock adapter. */
  todos: readonly TodoProjectionItem[]
...
/** Failed: terminal error ring with a cross. */
...
/** Blocked: unstarted ring with a terminal bar. */
...
function StatusGlyph({ status }: { status: TodoProjectionItem['status'] }) {
  switch (status) {
...
function progressLabel(todos: readonly TodoProjectionItem[], t: TodoPanelProps['t']): string {
  ...
  const pending = todos.length - done - failed - blocked - active
