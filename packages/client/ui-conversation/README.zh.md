...
无论会话规模大小，阅读者每次进入聊天流顶部 320px 时都会自动加载一页更早历史；页头操作仍保留为明确的重试与无障碍入口。每次请求都会在页面 prepend 时保持当前可见的语义行，并发的 Assistant 流式输出或响应式排版变化也不会改变该位置。只有阅读者仍停在底部时，流式增长才会跟随底部；向上滚动会立即释放跟随，回到底部则重新启用。分组后的消息流超过 100 行时，`@tanstack/react-virtual` 会把已挂载 DOM 限制为可见窗口与 overscan，且不改变这些滚动规则（[决策](../../../.agents/notes/implemented/feature/2026-08-18-chat-view-virtualization-and-autoload.zh.md)）。

聊天视图会把一个轮次的内部执行折叠为一行展开项，让消息流读起来像对话而不是执行日志。工具调用行、纯思考的 Assistant 步骤，以及——当轮次以最终回答收尾后——轮次中途的叙述文本，都会收进一行活动行（`已处理`，附用时与步数；成员仍在运行时为 `正在处理`，附实时计时），默认折叠；展开后在原位恢复原有节点行。流式中的 Assistant 步骤一旦开始产出正文就离开折叠，因此最终回答始终以流式可见；运行中的活动行会取代按设置语言显示的裸轮次状态（中文 `Voyaseek 思考中`，英文 `Voyaseek thinking`），成为工作中信号。其余节点——用户行、错误行、轮次尾部——仍是切分活动段的屏障。
