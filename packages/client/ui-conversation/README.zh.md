...
聊天视图会把一个轮次的内部执行折叠为一行展开项，让消息流读起来像对话而不是执行日志。工具调用行、纯思考的 Assistant 步骤，以及——当轮次以最终回答收尾后——轮次中途的叙述文本，都会收进一行活动行（`已处理`，附用时与步数；成员仍在运行时为 `正在处理`，附实时计时），默认折叠；展开后在原位恢复原有节点行。流式中的 Assistant 步骤一旦开始产出正文就离开折叠，因此最终回答始终以流式可见；运行中的活动行会取代裸轮次状态，成为工作中信号。其余节点——用户行、错误行、轮次尾部——仍是切分活动段的屏障。
