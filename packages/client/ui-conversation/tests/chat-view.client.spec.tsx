...
const retry = (seq: number): ModelRetryNode => ({
  ...
  failure: {
    code: 'RATE_LIMIT',
    status: 429,
    providerRetryAfterMs: 2_200,
    message: '{"error":{"message":"limit for key sk-retry-secret"}}',
  },
})
const turnError = (seq: number, category: TurnErrorNode['category']): TurnErrorNode => ({
  kind: 'turn-error', seq, time: seq * 1_000, turn: 1, step: 0,
  category,
  diagnostic: category === 'auth' ? 'Code: AUTH' : 'Code: UNKNOWN',
})
...
describe('ChatView', () => {
  ...
  it('animates only the latest unresolved model retry', () => {
    ...
    expect(disclosure.textContent).toContain('模型请求过于频繁')
    expect(disclosure.textContent).toContain('请在约 3 秒后重试')
    expect(disclosure.textContent).toContain('Code: RATE_LIMIT · HTTP: 429')
    expect(disclosure.textContent).not.toContain('sk-retry-secret')
  ...
  it('renders localized terminal failure guidance with diagnostics collapsed', () => {
    const h = makeHarness({ nodes: [user(1, 'try'), turnError(2, 'auth'), turnError(3, 'unknown')] })
    const view = render(<h.ChatView {...h.props} />)
    const statuses = view.getAllByRole('status')
    expect(statuses.map(status => status.textContent)).toEqual([
      '模型认证失败请检查 API Key 或登录凭据，更新后重试。',
      '模型请求失败请重试；若持续发生，请展开诊断信息并联系支持。',
    ])
    const diagnostics = [...view.container.querySelectorAll('details')]
    expect(diagnostics).toHaveLength(2)
    expect(diagnostics.every(detail => !detail.open)).toBe(true)
    fireEvent.click(diagnostics[0]!.querySelector('summary')!)
    expect(diagnostics[0]!.open).toBe(true)
    expect(diagnostics[0]!.textContent).toContain('Code: AUTH')
  })

  it('renders an actionable localized message for every stable failure category', () => {
    const categories: readonly TurnErrorNode['category'][] = [
      'quota',
      'rate-limit',
      'network',
      'timeout',
    ...
    const h = makeHarness({
      nodes: [user(1, 'try'), ...categories.map((category, index) => turnError(index + 2, category))],
    })
    const view = render(<h.ChatView {...h.props} />)
    const copy = view.getAllByRole('status').map(status => status.textContent)

    expect(copy).toEqual([
      '模型额度已用尽请检查账户额度或账单，恢复后重试。',
      '模型请求过于频繁模型服务正在限流，请稍后重试。',
