...
import type { TodoProjectionItem } from '@deepseek-ai/dsh-tool-todo/client'
...
const PARALLEL: TodoProjectionItem[] = [
  { content: '搭骨架', status: 'completed' },
  { content: '写组件', status: 'in_progress' },
...
describe('TodoPanel', () => {
  ...
  it('shows failed active work separately from blocked unstarted work', () => {
    render(<TodoPanel todos={[
      { content: '已交付', status: 'completed' },
      { content: '执行到一半', status: 'failed' },
      { content: '尚未开始', status: 'blocked' },
    ]} t={t} />)
    expect(screen.getByText('1 已完成 · 1 失败 · 1 已阻止（本轮未执行）')).toBeTruthy()
    expect(screen.queryByText(/进行中|待处理/)).toBeNull()

    fireEvent.click(screen.getByRole('button', { expanded: false }))
    expect(screen.getAllByRole('listitem').map(item => item.getAttribute('data-status')))
      .toEqual(['completed', 'failed', 'blocked'])
  })
})

/** Dock props stub: the adapter reads the 'todos' projection only; the rest of the owner share is unused. */
function dockProps(
  store: ReturnType<typeof createSnapshotStore<{ value: readonly TodoProjectionItem[] | null | undefined }>>,
): TodoDockProps {
...
describe('TodoDock', () => {
  it('reads the host-computed todos projection and follows pushed updates', () => {
    const store = createSnapshotStore<{ value: readonly TodoProjectionItem[] | null | undefined }>({ value: undefined })
