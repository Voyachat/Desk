...
describe('MessageItem arms', () => {
  ...
  it('user copy never claims success when the host rejects both write paths', async () => {
    Object.defineProperty(navigator, 'clipboard', {
      configurable: true,
  ...
  it('the snapshot form attributes each part to the subsystem that produced it', () => {
    const view = render(
      <MessageItem t={t} node={{
        ...
        source: {
          kind: 'plugin',
          plugin: '@voyaseek-ai/dsh-system-prompt',
          form: 'snapshot',
          sections: [{ name: 'sandbox:policy', text: 'workspace-write' }, { name: 'workspace', text: '/repo' }],
        },
        provenance: { role: 'inject', label: '@voyaseek-ai/dsh-system-prompt' },
        form: 'snapshot',
      } as never}
      />,
    )
    fireEvent.click(view.getByRole('button', { name: /^上下文注入\s*@voyaseek-ai\/dsh-system-prompt$/ }))
  ...
  it('collapses retry details behind the durable model retry status', () => {
    ...
    const view = render(
      <MessageItem
        ...
        node={{
          ...
          maxRetries: 2,
          delayMs: 2_500.4,
          failure: { code: 'TRANSPORT', message: '连接被重置：api_key=secret-value' },
        }}
      />,
    )
    ...
    expect(view.getByText('失败原因：').parentElement?.textContent).toBe(
      '失败原因：无法连接模型服务 — 请检查网络、代理和服务地址，然后重试。',
    )
    expect(view.getByText('Code: TRANSPORT')).toBeTruthy()
    expect(view.container.textContent).not.toContain('secret-value')
    expect(view.container.textContent).not.toContain('连接被重置')
