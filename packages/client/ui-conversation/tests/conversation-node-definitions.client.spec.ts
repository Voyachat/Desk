...
import type {
  ...
  ConversationNodeDefinition, ConversationViewDefinition, TurnErrorNode,
} from '@voyaseek-ai/dsh-client-runtime/client'
import { ConversationNodeAssembler } from '@voyaseek-ai/dsh-client-runtime/client'
...
describe('built-in conversation node Definitions', () => {
  ...
  it('projects raw provider failures into safe terminal-turn facts', () => {
    const raw = '{"error":{"code":429,"message":"quota for key AIza-provider-secret"}}'
    const value = assembler([
      at(1, 'turn/start', { turn: 1 }),
      at(2, 'step/start', { turn: 1, step: 1 }),
      ...
      at(4, 'turn/end', {
        ...
        reason: {
          ...
          error: {
            code: 'RATE_LIMIT',
            status: 429,
            providerRetryAfterMs: 2_500,
            requestId: 'req_gemini_429',
            message: raw,
          },
        },
      }),
    ])
    const failure = node(snapshot(value), 'turn-error')?.data as TurnErrorNode

    expect(failure).toMatchObject({
      kind: 'turn-error',
      category: 'rate-limit',
      retryAfterMs: 2_500,
      diagnostic: 'Code: RATE_LIMIT · HTTP: 429 · Retry-After: 2500ms · Request ID: req_gemini_429',
    })
    expect(JSON.stringify(failure)).not.toContain('AIza-provider-secret')
