...
**获取模型目录**会针对表单**当前显示**的端点调用 `llm.discoverModels`，包括已修改但尚未保存的 API 地址和已键入但尚未存储的密钥，因此新增一个提供方是一趟走完，而不是「先保存再回来」。OpenAI 兼容的 `GET /models` 回复只声明模型 ID，不能证明当前账号已开通每个模型，也不能证明每个模型都接受对话请求。因此回复会打开一个默认全部不勾选的选择框并说明这项区别；只有明确选中的模型才会写入，已配置的行仍优先于端点报告的容量。无法被询问的提供方只是绕路而非死路——适配器自己的消息会显示在各行旁边，而这些行仍可手工编辑。
