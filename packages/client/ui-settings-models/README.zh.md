...
每个已配置的 pi-ai 模型行都会显示按协议推导的 Native／Claude／Codex 兼容矩阵。Native 通过普通 LLM 适配器使用该路由；Codex 仅兼容 `openai-responses`，Claude 仅兼容 `anthropic-messages`，未设置协议时保持未确认。这些标签描述的是进程协议准入，不代表账户已开通或 endpoint 可达。兼容的替代 Runtime 路由还必须具有已解析 endpoint 与 `apiKeyEnv`；Host 会复用该凭据引用，但不会把密钥返回浏览器。
