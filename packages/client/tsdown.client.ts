 ...
 * with no runtime identity consumed across bundles (no Symbol/instanceof/singleton state).
 * The Aistaff Employee Experience entry adds a Service base used only by its
 * owning Remote provider; consumers collaborate through the registered service
 * and never compare that class identity across bundles.
 * Everything else under @voyaseek-ai/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@voyaseek-ai\/dsh-(?:aistaff-employee-experience|host-apiproxy|session|llm|tools|brand)(?:\/|$)|@voyaseek-ai\/dsh-aistaff-local-capability\/object-layer$)/
...
const GENERATED_REMOTE = /^@voyaseek-ai\/dsh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
 ...
 */
const RUNTIME_STORE_EXEMPTION = '@voyaseek-ai/dsh-client-runtime/client'
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // and every other @voyaseek-ai value import is a build error — a
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        const virtualId = CSS_VIRTUAL_PREFIX + repositoryAssetId(abs) + CSS_VIRTUAL_SUFFIX
        const existing = cssFiles.get(virtualId)
        if (existing !== undefined && existing !== abs) {
          throw new Error(`client CSS virtual id collision: ${virtualId}`)
        }
      ...
      async load(virtualId: string) {
        ...
        if (fileId === undefined) throw new Error(`missing CSS source for ${virtualId}`)
...
/** Name a physical stylesheet without embedding its build-machine path in the browser bundle. */
function repositoryAssetId(fileId: string): string {
  const repositoryPath = relative(REPOSITORY_ROOT, fileId).split(sep).join('/')
  if (repositoryPath.length === 0 || repositoryPath.startsWith('../') || isAbsolute(repositoryPath)) {
    throw new Error('client CSS source must be inside the repository')
  }
