...
import type { TypertContribution } from '@voyaseek-ai/dsh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@voyaseek-ai/dsh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven dsh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@voyaseek-ai/dsh-tools'],
    }).analyze()
    ...
    const artifact = new FaceModelEmitter(host).emit('@voyaseek-ai/dsh-tools')
    ...
    const record = ctx.typert.getPackage('@voyaseek-ai/dsh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@voyaseek-ai/dsh-tools', 'host')).toBeUndefined()
  })
})
