...
import { ApprovalRequestId } from '@voyaseek-ai/dsh-user-approval'
import * as ApprovalInvariant from '@voyaseek-ai/dsh-user-approval/invariant'
...
describe('approval invariants', () => {
  it('accepts paired audit events and closed policy values', async () => {
    ...
    session.append('approval/asked', { id, toolName: 'bash', rememberable: true })
    session.append('approval/decided', { id, outcome: 'allowed-and-remembered' })
  ...
  it('rejects malformed and unpaired audit events', async () => {
    ...
    expect(() => session.append('approval/decided', { id, outcome: 'allowed-and-remembered' }))
