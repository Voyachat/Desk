...
Channel-neutral approval seam. `ctx.approval.request(req)` returns `allowed-once`, `allowed-and-remembered`, `rejected`, `cancelled`, or `unavailable`; missing or failing answerers fail closed. A remembered outcome is valid only when the requester advertises `rememberable: true`, and that requester owns applying its exact permission update. Exact event signatures live in the generated region of [approval.md](../../../docs/subsystems/approval.md#cordis-surface).
...
- **Remembered permissions are requester-owned** — this seam records the decision but has no grant store or rule synthesis. Only a requester that advertises an exact update may receive `allowed-and-remembered`; revocation remains the requester's responsibility. Session policy is still only `ask` / `never`.
