...
The service owns the `permission` Settings namespace. `defaultPreset` is the global default for future sessions; `workspacePresets` maps canonical absolute workspace paths to project-specific defaults. A fresh session resolves its canonical cwd in `workspacePresets` before falling back to `defaultPreset`; creation pins `permission/preset`, `sandbox/mode`, and `approval/policy` into the session, so later Settings changes never alter an existing session. The composition entry supplies `Config.defaultPreset`, or the preset matching the composed sandbox and approval defaults when omitted. A resumed seed, including an explicitly empty one marked by `session/end-seed`, preserves its effective permission and receives only missing durable facts rather than either current default. Mounting the service also sweeps already-live sessions, so an HMR replacement pins any session created while the plugin was absent.

The file-backed provider stores both levels in the user-owned `$VOYASEEK_HOME/settings.yaml`, for example `permission: { defaultPreset: workspace-write, workspacePresets: { /absolute/project: read-only } }`. Workspace keys must already be canonical absolute paths and every value must name a configured preset. Repository-owned instruction or settings files are not an authorization source: a checked-out project cannot grant itself wider access.
...
- **Stored defaults must remain in the preset table** — removing a preset referenced by `defaultPreset` or `workspacePresets` makes Permission settings registration fail until the `permission` section in `settings.yaml` is updated or reset.
- **File effects only** — these presets do not control network access and their UI must not claim that they do.
