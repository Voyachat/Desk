...
该服务拥有 `permission` Settings namespace。`defaultPreset` 是未来会话的全局默认值；`workspacePresets` 将规范绝对工作区路径映射为项目默认值。新会话先用其规范 cwd 查找 `workspacePresets`，未命中时再回退到 `defaultPreset`；创建过程将 `permission/preset`、`sandbox/mode` 和 `approval/policy` 固定到该会话中，因此后续 Settings 变更绝不会改变现有会话。组合项使用 `Config.defaultPreset`；省略时，则推断与组合后的沙箱和审批默认值匹配的 preset。恢复的 seed，包括由 `session/end-seed` 标记的显式空 seed，都会保留其有效权限，只补齐缺失的持久事实，而不会采用任一当前默认值。挂载服务时还会遍历所有已存活会话，因此 HMR（热模块替换）会固定插件缺席期间创建的所有会话。

文件设置提供方将两个层级都存入用户拥有的 `$VOYASEEK_HOME/settings.yaml`，例如 `permission: { defaultPreset: workspace-write, workspacePresets: { /absolute/project: read-only } }`。工作区 key 必须已经是规范绝对路径，每个值都必须命名一个已配置的 preset。仓库内的指令或设置文件不是授权来源：检出的项目不能自行授予更宽权限。
...
- **已存储的默认值必须保留在 preset 表中**：移除 `defaultPreset` 或 `workspacePresets` 引用的 preset 会导致权限设置注册失败，直到更新或重置 `settings.yaml` 中的 `permission` 分节。
- **只控制文件效果**：这些 preset 不控制网络访问，UI 不能声称其控制网络。
