...
import * as SessionStatsPlugin from '@voyaseek-ai/dsh-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@voyaseek-ai/dsh-session-projection', SessionProjectionRegistry],
    ['@voyaseek-ai/dsh-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@voyaseek-ai/dsh-session-projection'",
      "- name: '@voyaseek-ai/dsh-session-stats'",
    ])
