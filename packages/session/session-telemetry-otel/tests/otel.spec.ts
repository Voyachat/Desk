...
import { getOrCreateAnonymousUserId } from '@voyaseek-ai/dsh-anonymous-user-id'
import Loader from '@voyaseek-ai/cordis-plugin-loader'
import { recordFeedback } from '@voyaseek-ai/dsh-command-feedback'
...
// pin VOYASEEK_HOME to a temp dir so the suite never touches the ambient ~/.dsh.
let tempHome: string
let previousDshHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'dsh-otel-home-'))
  previousDshHome = process.env.VOYASEEK_HOME
  process.env.VOYASEEK_HOME = tempHome
})
afterAll(() => {
  if (previousDshHome === undefined) delete process.env.VOYASEEK_HOME
  else process.env.VOYASEEK_HOME = previousDshHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'voyaseek-harness' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@voyaseek-ai/dsh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@voyaseek-ai/dsh-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@voyaseek-ai/dsh-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
