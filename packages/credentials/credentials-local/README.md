...
| `$VOYASEEK_HOME/.credentials.yaml` document | `file` | yes (`set`/`unset`) | over both `.env` layers |
| `<invocation cwd>/.env` | `project-env` | not here | over the user `.env` |
| `$VOYASEEK_HOME/.env` | `user-env` | not here | otherwise |
...
| `dshHome` | `$VOYASEEK_HOME` or `~/.voyaseek` | Harness home used when `path` is omitted. |
