...
```
@voyaseek-ai/cordis-plugin-timer  timer service (writes nothing to stdout)
@voyaseek-ai/dsh-llm              abstract LLM service + content-block vocabulary
@voyaseek-ai/dsh-session          event-sourced session log + store
@voyaseek-ai/dsh-session-title    log-backed title service + deterministic fallback
@voyaseek-ai/dsh-system-prompt    prompt-section + tool-schema assembly
@voyaseek-ai/dsh-tools            registry + guarded pre/around/post/final-result pipeline
@voyaseek-ai/dsh-skill            skill provider registry
@voyaseek-ai/dsh-skill-filesystem      local filesystem skill provider
@voyaseek-ai/dsh-agent            agent registry + initiator scope + agent/* events
@voyaseek-ai/dsh-goal             optional persisted same-session goal domain
@voyaseek-ai/dsh-tool-goal        optional model-facing goal controls
@voyaseek-ai/dsh-goal-round-driver     optional same-session goal-round driver
@voyaseek-ai/dsh-llm-retry        provider-routed request retry policy
@voyaseek-ai/dsh-jobs-local      generic background-job registry
@voyaseek-ai/dsh-invariants       configurable invariant registry service
@voyaseek-ai/dsh-session/invariant
@voyaseek-ai/dsh-agent/invariant
@voyaseek-ai/dsh-scope/invariant
@voyaseek-ai/dsh-agent-loop/invariant
                                  package-owned relational checks
@voyaseek-ai/dsh-tool-bash        the model-facing bash schema (unless toolBash=false)
@voyaseek-ai/dsh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@voyaseek-ai/dsh-tool-skill       session-prefix skill catalog + model-facing loader schema
@voyaseek-ai/dsh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@voyaseek-ai/dsh-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@voyaseek-ai/dsh-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@voyaseek-ai/dsh-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.md)。
