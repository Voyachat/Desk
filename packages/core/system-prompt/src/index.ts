 ...
 *
 * @module @voyaseek-ai/dsh-system-prompt
 */

import { Context, Service } from '@voyaseek-ai/cordis'
import z from '@voyaseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@voyaseek-ai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@voyaseek-ai/dsh-scope'
import type { ContextSnapshotSection, ToolSchema } from '@voyaseek-ai/dsh-llm'
...
declare module '@voyaseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@voyaseek-ai/dsh-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by Voyaseek Harness.',
      })
    }
    this.section({
