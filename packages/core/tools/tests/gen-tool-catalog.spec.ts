...
describe('gen-tool-catalog collectToolCatalog', () => {
  it('boots every shipped tool package and harvests its model-facing schemas', async () => {
    ...
    expect(names).toEqual(['ask_user_question', 'bash', 'bash', 'complex_goal', 'cordis_define', 'cordis_inspect_list', 'cordis_inspect_query', 'cordis_inspect_self', 'cordis_run', 'cordis_stop', 'cordis_undefine', 'create_goal', 'edit', 'exit_plan_mode', 'find_dsh_plugin', 'get_goal', 'glob', 'grep', 'interrupt_agent', 'job_kill', 'job_list', 'job_output', 'list_agents', 'lsp', 'modelscope_search', 'pwsh', 'ralph', 'read', 'read_image', 'report', 'run_code', 'schedule_create', 'schedule_delete', 'schedule_list', 'send_message', 'session_event_read', 'session_event_search', 'session_event_trace', 'session_search', 'session_trace', 'skill', 'str_replace_editor', 'subagent', 'terminal_close', 'terminal_list', 'terminal_open', 'terminal_read', 'terminal_send', 'terminal_signal', 'todo_write', 'update_goal', 'web_fetch', 'web_search', 'workflow', 'write'])
  ...
  it('attributes each harvested tool with its registering plugin source', async () => {
    const catalog = await collectToolCatalog()
    const bash = catalog.find(entry => entry.pkg === '@voyaseek-ai/dsh-tool-bash')
    expect(bash?.sources.bash).toBe('packages/shell/tool-bash/src/index.ts')
    const control = catalog.find(entry => entry.pkg === '@voyaseek-ai/dsh-tool-subagent-control')
  ...
  it('harvests search tools without depending on the generator process PATH', async () => {
    ...
    try {
      process.env.PATH = ''
      const catalog = await collectToolCatalog()
      const search = catalog.find(entry => entry.pkg === '@voyaseek-ai/dsh-tool-fs-search')
  ...
  it('records the shipped `subagent_fork` alias in a note (config-driven tool name)', async () => {
    ...
    const catalog = await collectToolCatalog()
    const subagent = catalog.find(entry => entry.pkg === '@voyaseek-ai/dsh-tool-subagent')
...
describe('gen-tool-catalog assertToolsHarvested', () => {
  ...
  it('throws, naming the package and its requirements, when a boot registers nothing', () => {
    ...
    expect(() => { assertToolsHarvested(entry, 0) }).toThrow(/@voyaseek-ai\/dsh-tool-demo booted without registering a single tool/)
...
describe('gen-tool-catalog render', () => {
  it('emits a package heading, a tool heading, and a json schema fence', () => {
    ...
    expect(md).toContain('| `@voyaseek-ai/dsh-tool-demo` | `demo` | `ctx.tools` | `tool/result` |')
    expect(md).toContain('## `@voyaseek-ai/dsh-tool-demo`')
