...
import { CodeRuntime } from '@voyaseek-ai/dsh-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@voyaseek-ai/dsh-code-runtime'
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@voyaseek-ai/dsh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@voyaseek-ai/dsh-tools'
import type { Agent } from '@voyaseek-ai/dsh-agent'
import { Session, SessionId } from '@voyaseek-ai/dsh-session'
import type { JsonValue, SessionEventMap } from '@voyaseek-ai/dsh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@voyaseek-ai/dsh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
