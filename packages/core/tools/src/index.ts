 ...
 * execution pipeline.
 * @module @voyaseek-ai/dsh-tools
 */

import { Context, Service } from '@voyaseek-ai/cordis'
import z from '@voyaseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@voyaseek-ai/dsh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@voyaseek-ai/dsh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@voyaseek-ai/dsh-llm'
import { assertNever, deepFreeze, HarnessError } from '@voyaseek-ai/dsh-llm'
import type { Agent } from '@voyaseek-ai/dsh-agent'
import { snapshotJsonValue } from '@voyaseek-ai/dsh-session'
import type { JsonValue, UserMessage } from '@voyaseek-ai/dsh-session'
import type { ToolProviderResult } from '@voyaseek-ai/dsh-system-prompt'
import type { CodeRuntime } from '@voyaseek-ai/dsh-code-runtime'
...
export type { JsonValue } from '@voyaseek-ai/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@voyaseek-ai/dsh-tools`
...
declare module '@voyaseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@voyaseek-ai/dsh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@voyaseek-ai/dsh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@voyaseek-ai/dsh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@voyaseek-ai/dsh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@voyaseek-ai/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @voyaseek-ai/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
