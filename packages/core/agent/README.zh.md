...
可选配套包 `@voyaseek-ai/dsh-agent/invariant` 会向 `ctx.invariants` 注册此包的 agent（智能体）状态转换检查。根 agent 服务不会隐式加载诊断。
...
`AgentOptions` 提供初始的提供方／模型路由，以及可选的正数 `maxTokens` 输出上限。替代 Driver 可以通过 `modelConstraint` 公布默认路由与额外的准确 provider／model 子集；模型选择和 Driver 共用 `modelConstraintAllows()` 判定，因此显示为可选的条目无法绕过 Runtime 协议准入。具体循环会解析确切模型的适配器默认值，把生效上限记录到请求 header，并应用到每次对话模型请求；显式 Agent 选项优先，省略时由适配器或提供方路由默认值控制。
