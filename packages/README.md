...
npm scope: `@voyaseek-ai/dsh-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@voyaseek-ai/dsh-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`memory/`](memory/README.md) | Cross-session memory capability: provider-neutral service, bounded local provider, and automatic capture/recall Consumer | Product — stable API |
...
| [`document/`](document/README.md) | Document-to-Markdown seam and local OCR providers | Product — stable API |
