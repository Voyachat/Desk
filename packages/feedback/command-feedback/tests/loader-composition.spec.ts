...
import type { Agent, AgentStatus } from '@voyaseek-ai/dsh-agent'
import CommandRuntime from '@voyaseek-ai/dsh-commands'
import SessionStore, { SessionId } from '@voyaseek-ai/dsh-session'
import * as CommandFeedback from '@voyaseek-ai/dsh-command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@voyaseek-ai/dsh-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { VOYASEEK_HOME: root } })
