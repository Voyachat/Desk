...
import Storage from '@voyaseek-ai/dsh-storage'
import * as StorageDomain from '@voyaseek-ai/dsh-storage-domain'
import * as StorageJson from '@voyaseek-ai/dsh-storage-json'
import { remoteMethods } from '@voyaseek-ai/dsh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@voyaseek-ai/dsh-session', SessionStore],
    ['@voyaseek-ai/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@voyaseek-ai/dsh-storage', Storage],
    ['@voyaseek-ai/dsh-storage-json', StorageJson],
    ['@voyaseek-ai/dsh-storage-domain', StorageDomain],
    ['@voyaseek-ai/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@voyaseek-ai/dsh-session'",
      "- name: '@voyaseek-ai/dsh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@voyaseek-ai/dsh-storage'",
      "- name: '@voyaseek-ai/dsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@voyaseek-ai/dsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@voyaseek-ai/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
