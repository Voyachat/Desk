...
/**
 * Keyless built-artifact smoke: plain Node imports `@voyaseek-ai/dsh-lsp` and
 * `@voyaseek-ai/dsh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      const { Context } = await import('@voyaseek-ai/cordis')
      const { default: Lsp } = await import('@voyaseek-ai/dsh-lsp')
      const LspLocal = await import('@voyaseek-ai/dsh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@voyaseek-ai/dsh-fs-local')
      const { default: LocalSubprocessRuntime } = await import('@voyaseek-ai/dsh-subprocess-local')
