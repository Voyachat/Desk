...
npm scope 为 `@voyaseek-ai/dsh-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@voyaseek-ai/dsh-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`memory/`](memory/README.md) | 跨会话 memory 能力：提供方中立服务、有界本地提供方，以及自动捕获／召回 Consumer | 产品：稳定 API |
...
| [`document/`](document/README.md) | 文档转 Markdown seam 与本机 OCR provider | 产品：稳定 API |
