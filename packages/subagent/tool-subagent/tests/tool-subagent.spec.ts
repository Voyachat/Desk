...
import { assembleContextFor, type Agent } from '@voyaseek-ai/dsh-agent'
...
import type { SubagentStartRequest } from '@voyaseek-ai/dsh-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @voyaseek-ai/dsh-jobs')
  })
