...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@voyaseek-ai/dsh-llm', LlmRuntime],
    ['@voyaseek-ai/dsh-session', SessionStore],
    ['@voyaseek-ai/dsh-token-meter', TokenMeter],
    ['@voyaseek-ai/dsh-compaction-tool-result-pruner', ToolResultPruner],
    ['@voyaseek-ai/dsh-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      "- name: '@voyaseek-ai/dsh-llm'",
      "- name: '@voyaseek-ai/dsh-session'",
      "- name: '@voyaseek-ai/dsh-token-meter'",
      "- name: '@voyaseek-ai/dsh-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
      "- name: '@voyaseek-ai/dsh-compaction-basic'",
      '  config:',
      '    thresholdRatio: 0.5',
      '    retainRatio: 0.125',
      '    auto: false',
