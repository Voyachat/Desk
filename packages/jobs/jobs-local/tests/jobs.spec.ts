...
import { bindScopeParent, createScope, scopeOf } from '@voyaseek-ai/dsh-scope'
import type { ScopeKey } from '@voyaseek-ai/dsh-scope'
import { JobId } from '@voyaseek-ai/dsh-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@voyaseek-ai/dsh-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @voyaseek-ai/dsh-tool-jobs in its composition)')
  })
