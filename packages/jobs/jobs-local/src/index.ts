 ...
 * @module @voyaseek-ai/dsh-jobs-local
 */

import { Context } from '@voyaseek-ai/cordis'
import z from '@voyaseek-ai/schemastery'
import type { Agent } from '@voyaseek-ai/dsh-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@voyaseek-ai/dsh-scope'
import type { ScopeLayer } from '@voyaseek-ai/dsh-scope'
import { deadline, timeoutOf } from '@voyaseek-ai/dsh-timeout'
import { JobRegistry, JobId } from '@voyaseek-ai/dsh-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@voyaseek-ai/dsh-jobs'
 ...
 * `@voyaseek-ai/dsh-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @voyaseek-ai/dsh-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @voyaseek-ai/dsh-agent)')
    }
