 ...
 * process-local registry lives in `@voyaseek-ai/dsh-jobs-local`.
 * @module @voyaseek-ai/dsh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@voyaseek-ai/dsh-jobs is the abstract job registry seam; load an implementation such as @voyaseek-ai/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
