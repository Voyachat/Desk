...
Some wire facts belong to the endpoint rather than to any profile, and materialization applies them after the compat chain above so they win. DashScope's OpenAI-compatible endpoint refuses the `developer` message role, while pi-ai's URL-derived detection recognizes the host only as an OpenAI-compatible endpoint and sends `developer` for every reasoning model's system prompt — failing the model with HTTP 400 once an agent request carries one. Models served through `dashscope.aliyuncs.com` and `dashscope-intl.aliyuncs.com` therefore materialize with `supportsDeveloperRole` disabled and their system prompts travel as the `system` role every OpenAI-compatible endpoint accepts.

A model neither the entry nor the installed catalog sizes asks the model-family knowledge base before falling back to the route's `defaultContextWindow` (262,144) and `defaultMaxTokens` (32,768), so a listing that discloses nothing but ids still yields a serviceable route. The route fallbacks are guesses by construction, which is why they are route fields a deployment whose gateway serves smaller models corrects once rather than constants buried in the adapter; a fallback sizes the model and never becomes a per-request cap. The same knowledge base lends a hand-declared model its family's input modalities and selectable reasoning efforts, sitting below an explicit entry and above the route defaults in the same resolution order; the Model-family knowledge section owns the classifier.
...
Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `alternateEndpoints`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, and `retryPolicy`. `alternateEndpoints` accepts at most one endpoint per protocol distinct from the primary `api`; every entry shares the route's credential reference. Each profile's optional retry policy is captured with that provider route; omission uses bounded normal defaults. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Harness app attribution wins a conflicting configured header name.
...
Most listings disclose an id and nothing else; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them, entries without a usable id are skipped rather than failing the whole listing, and everything else the adopting surface still owes. A wire listing is additionally narrowed to the models an agent conversation can use — endpoints such as DashScope list their embeddings, image generators, and TTS voices beside their chat models — and known chat families lend the capacities their listing withheld, so an adopted row materializes with its real context window instead of the route's guess; the Model-family knowledge section owns both decisions. A route answered from the installed catalog skips the classifier: pi-ai's registry is the authority for its own providers. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.

## Model-family knowledge

OpenAI-compatible endpoints answer `GET /models` with every model they host and nothing but ids: DashScope lists text embeddings, TTS voices, image generators, realtime websocket models, and tool-less specialty models beside its chat models. Adopting such a listing wholesale puts models an agent cannot use beside the ones it can, and serves the usable ones with guessed capacities. A curated knowledge base classifies each model id and corrects both, and it is the one place this package encodes facts about specific model families.

The stance is asymmetric. A **known non-chat family** — embeddings, rerankers, image and video generators, speech synthesis and recognition, realtime websocket models, internal infrastructure models — and a **known tool-less chat family**, which refuses the tool calls every agent request carries, are refused: endpoint interrogation drops them from the adoption candidates, and `listModels` leaves them out of the conversation directory. A model the knowledge base does not know stays servable in both places: hiding an unknown id would break a gateway this base has never seen, while admitting one the endpoint cannot serve fails with the provider's own diagnostic. Two answers outrank the classifier everywhere: a profile entry that declares its own `input` modalities states what its endpoint serves, and a model the installed pi-ai catalog ships is pi-ai's own claim. Membership stays advisory — a hidden model still dispatches when a session selects one, so nothing here can strand a running conversation.

A **known chat family** lends the capabilities its listing would not disclose: context window, output cap, input modalities, and the selectable reasoning efforts of hybrid-reasoning models. Materialization applies them only to a model the installed catalog does not describe, in the field-by-field order entry → installed catalog → family facts → route defaults, so an explicit profile always wins and a catalog route is never touched. Lookup normalizes the id before matching: lowercase, owner prefix removed (`kimi/kimi-k3` as `kimi-k3`), and snapshot suffixes stripped one at a time (`qwen3-max-2026-01-23` as `qwen3-max`, `deepseek-v4-pro-0813` as `deepseek-v4-pro`), with the exact id leading so a dated entry wins over its family. Chat capacities come from the upstream pi-ai qwen-token-plan catalog, which describes the same families on Alibaba Cloud's OpenAI-compatible endpoints; reasoning sets come from endpoint probes that answered a streamed chat completion with tool definitions. A family fact is an advisory default, never a claim: a gateway that serves the same id differently is corrected with the same entry fields that outrank it.
...
The route's primary protocol and `alternateEndpoints` control alternative-runtime admission. Codex uses the route's `openai-responses` endpoint and Claude uses its `anthropic-messages` endpoint when that endpoint is non-empty and the route names `apiKeyEnv`; both resolve the same credential reference immediately before the child starts. The adapter exports the selected endpoint and reference, never the key. A protocol absent from the route remains unavailable instead of being guessed from a model id or successful `/models` listing.

An explicit runtime connectivity probe sends `Reply with OK.` with a one-token output limit through the exact saved provider, model, and protocol. Native uses the ordinary LLM stream; Codex and Claude use their selected endpoint directly with the route's resolved credential. The probe never reads or retains a provider response body and reports only provider-neutral failure codes, including HTTP status classes, transport failure, timeout, and missing credentials. It is a real provider request and runs only when a caller explicitly invokes it.
...
- **Model-family knowledge is a curated snapshot** — the classifier encodes the non-chat and tool-less families plus the chat capacities known at release, and a family released later resolves as unknown: servable, with the route's capacity guesses and no reasoning metadata, until the table learns it. A model misclassified as non-chat is recovered per route by declaring its `input` modalities on its entry.
- **Native dispatch still uses one primary protocol per route** — `api` and `baseURL` select the descriptor used by ordinary LLM requests. `alternateEndpoints` admit the same model catalog and credential to Codex or Claude; they do not assign different Native models to different protocols or prove that the provider exposes every configured model on each endpoint.
