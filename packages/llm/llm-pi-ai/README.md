...
That exact descriptor also controls alternative-runtime admission. A model using `openai-responses` can be handed to Codex, while a model using `anthropic-messages` can be handed to Claude, only when its resolved endpoint is non-empty and the route names `apiKeyEnv`. The adapter exports the endpoint and credential reference, not the key; each driver resolves the key immediately before its child starts. Other protocols remain Native-only instead of being guessed compatible from a model id or a successful `/models` listing.
