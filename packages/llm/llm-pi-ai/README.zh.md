...
条目与已安装 catalog 都没有给出尺寸的模型，会先查模型族知识库，再回退到该路由的 `defaultContextWindow`（262,144）与 `defaultMaxTokens`（32,768），因此一份只公布 id 的列表同样能产出可服务的路由。路由回退值本质上都是猜测，这正是它们作为路由字段、供网关服务更小模型的部署一次性更正的原因，而不是埋在适配器里的常量；回退值只用于给模型定尺寸，绝不会变成单次请求上限。同一个知识库还会为手工声明的模型补上其所属族的输入模态与可选推理档位，在解析顺序中位于显式条目之下、路由默认之上；分类器本身见[模型族知识库](#模型族知识库)。
...
多数列表只公布 id；`context_window`/`context_length` 与 `max_output_tokens`/`max_tokens` 在网关提供时会被读取，没有可用 id 的条目会被跳过而不是让整份列表失败，其余仍由采纳方补齐。来自协议的列表还会被收窄到 agent 对话可用的模型——DashScope 这类端点会把 embedding、图片生成、TTS 音色与聊天模型列在同一份清单里——已知聊天族会补上列表没有公布的容量，使采纳的条目以其真实上下文窗口物化，而不是路由的猜测值；两个决定都由[模型族知识库](#模型族知识库)负责。从已安装 catalog 作答的路由不经过分类器：pi-ai 的注册表是它自己提供方的权威。回复在四兆字节上限下读取，且上限落在实际收到的字节上——端点是用户自己填的 URL，因此会先看声明长度，但绝不把它当作边界。端点不可达、凭据被拒、响应非 JSON、以及响应没有 `data` 数组，都会以 `DISCOVERY_FAILED` 失败，消息点名端点；仅当 401 或 403 时才点名凭据。读取响应体期间被取消会呈现为 `ABORTED`，与请求发出之前被取消一致。

## 模型族知识库

OpenAI 兼容端点对 `GET /models` 的回答是它托管的全部模型，而且往往只给 id：DashScope 会把文本 embedding、TTS 音色、图片生成、实时 websocket 模型，以及不支持工具的专用模型，和它的聊天模型列在同一份清单里。整份采纳这样的列表，会把 agent 用不了的模型放到能用的模型旁边，同时让能用的模型带着猜出来的容量上岗。一个精心维护的知识库对每个模型 id 做分类，并同时纠正这两件事；这也是本包唯一对具体模型族编码事实的地方。

立场是不对称的。**已知的非聊天族**——embedding、rerank、图片与视频生成、语音合成与识别、实时 websocket 模型、内部基建模型——以及**已知的无工具聊天族**（它们拒绝每个 agent 请求都会携带的工具调用）会被拒绝：端点询问会把它们从采纳候选中剔除，`listModels` 也不把它们放进对话目录。知识库不认识的模型在两处都保持可服务：隐藏一个未知 id 会弄坏这个知识库从未见过的网关，而放行一个端点服务不了的模型，失败时带着提供方自己的诊断信息。有两个答案在任何地方都优先于分类器：声明了自己 `input` 模态的 profile 条目，说的是它的端点实际服务什么；已安装 pi-ai catalog 收录的模型，那是 pi-ai 自己的声明。目录成员身份依然是建议性的——被隐藏的模型一旦被某个会话选中仍然会分发，因此这里没有任何东西能让一个进行中的对话断掉。

**已知聊天族**会补上列表不肯公布的能力：上下文窗口、输出上限、输入模态，以及混合推理模型可选的推理档位。物化只把这些事实应用到已安装 catalog 未描述的模型上，按字段依次取 条目 → 已安装 catalog → 族事实 → 路由默认，因此显式 profile 总是赢，catalog 路由从不被触碰。查找前先归一化 id：转小写、去掉 owner 前缀（`kimi/kimi-k3` 按 `kimi-k3` 查）、逐级剥掉快照后缀（`qwen3-max-2026-01-23` 按 `qwen3-max`，`deepseek-v4-pro-0813` 按 `deepseek-v4-pro`），且精确 id 优先，因此带日期的条目赢过它的族。聊天容量取自上游 pi-ai 的 qwen-token-plan catalog，它描述的是阿里云 OpenAI 兼容端点上的同一批族；推理集合来自对「带工具定义的流式聊天补全」作出应答的端点探测。族事实是建议性的默认，绝不是断言：某个网关用不同方式服务同一个 id 时，用同一批优先级更高的条目字段更正即可。
