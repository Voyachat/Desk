...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@voyaseek-ai/dsh-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@voyaseek-ai/dsh-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@voyaseek-ai/dsh-llm-retry', retry],
    ['@voyaseek-ai/dsh-agent-loop', AgentLoop],
  ])
...
describe('real Loader composition', () => {
  ...
  it('loads provider-supplied policy and records recovery through the shipping loop', { timeout: 60_000 }, async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@voyaseek-ai/dsh-llm-retry'",
