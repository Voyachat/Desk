...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@voyaseek-ai/dsh-anonymous-user-id'
import { SessionId } from '@voyaseek-ai/dsh-session'
import * as LlmDeepSeek from '@voyaseek-ai/dsh-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@voyaseek-ai/dsh-llm-deepseek'
...
beforeEach(() => {
  ...
  vi.stubEnv('VOYASEEK_HOME', testHome)
})
...
describe('DeepSeekAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-voyaseek-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-voyaseek-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-voyaseek-harness-compact')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-voyaseek-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-voyaseek-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-voyaseek-harness-compact']).toBe('1')
  })
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.voyaseek/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
