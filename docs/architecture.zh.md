...
| [`document/document-converter`](../packages/document/document-converter/README.md) | 与提供方无关的文档转 Markdown 能力 | `ctx.documentConverter` |
...
跨会话记忆是独立的 Provider seam。默认 Consumer 从 `session/event` 采集已完成轮次，并通过 `agent/pre-step` 召回；召回历史以插件来源的 `user/message` 进入，因此沿用同一套持久投影，而不建立第二份模型上下文存储。存储与用户管理属于 `ctx.agentMemory`，不属于会话日志或 agent loop。
...
| 添加本机 OCR 或文档转换 | 注册 `ctx.documentConverter` provider；Host 图片降级会在托管视觉模型前消费它 |
...
| 添加或替换跨会话记忆 | 注册一个 `ctx.agentMemory` Provider；自动采集与召回仍然消费 Session 和 Agent 事件 |
