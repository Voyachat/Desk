...
`Agent` 是每个插件（UI、钩子、orchestrator）面向编程的 surface；`ctx.agents.get(id)` 返回它，[发起者作用域](#initiating-agent)携带它。具体实现为 dsh-agent-loop 包内部细节；循环外没有任何组件依赖它。统一的 `send` 方法直接暴露 target 与 wakeup 路由；`followup`、`steer` 与 `inject` 是固定预设的别名方法。替代 runtime 可以公开 `modelConstraint`，将模型选择限制在其原生协议能够服务的提供方／模型路由内。
...
Agent service (`ctx.agents`): tracks live agents and carries the initiating Agent through one process-local asynchronous driver chain. Agent *creation* is provided by whichever plugin implements the AgentFactory (`@voyaseek-ai/dsh-agent-loop`), registered via setFactory.
...
Source: [`packages/core/agent/src/runtime-types.ts:329`](../../packages/core/agent/src/runtime-types.ts)
...
Source: [`packages/core/agent/src/runtime-types.ts:317`](../../packages/core/agent/src/runtime-types.ts)
