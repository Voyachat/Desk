...
默认 Consumer 先排队每个已完成轮次，再通过 `ctx.llm` 使用该会话已路由模型，针对相关候选生成并验证 upsert、delete 或 no-op mutation。用户文本是权威来源，Assistant 回答只用于消歧。Provider 事务提交后，一条持久维护事件会记录发生变化的精确条目 ID。同一 Consumer 会在后续会话的第一个模型步骤前调用 `recall()`，并提供显式搜索、记住与遗忘工具。召回文本是一条不可信、来源为 `agent-memory` 的 `user/message`，因此普通会话投影会准确记录模型收到的内容，浏览器也能提供精确条目纠正入口。
