 ...
 * REAL `@voyaseek-ai/dsh-sdk-client`, drives one turn over stdio JSON-RPC,
...
import { VoyaseekHarness, type HarnessNotification, type RunResult } from '@voyaseek-ai/dsh-sdk-client'
...
function assembledRuntimeContexts(log: PersistedLog): string[] {
  return log.content.trimEnd().split('\n').flatMap((line) => {
    ...
    if (event.type !== 'user/message'
      ...
      || event.data.source.plugin !== '@voyaseek-ai/dsh-system-prompt') return []
