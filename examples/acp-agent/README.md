...
Stdout carries only newline-delimited ACP JSON-RPC. `@voyaseek-ai/dsh-acp-demo` installs no stdout logger; leaf additions must use stderr for diagnostics.

The automation contract — supported methods, baseline prompt content, committed-text output, and the intentionally absent UI surfaces — lives in [`@voyaseek-ai/dsh-acp`](../../packages/acp/acp/README.md).
