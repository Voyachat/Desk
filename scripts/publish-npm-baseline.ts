...
const RELEASE_ENTRY_PACKAGE = '@voyaseek-ai/dsh'
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@voyaseek-ai/')) {
        throw new Error(`${manifestPath} must name an @voyaseek-ai package`)
      }
      if (name === '@voyaseek-ai/dsh-root') {
...
class InstalledBundleSmoke {
  ...
  run(): void {
    ...
    try {
      ...
      const bin = resolve(consumerRoot, 'node_modules/@voyaseek-ai/dsh/lib/bin.js')
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@voyaseek-ai/') || name === '@voyaseek-ai/dsh-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.VOYASEEK_HOME = resolve(consumerRoot, '.voyaseek')
