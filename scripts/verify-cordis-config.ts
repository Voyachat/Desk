...
const CHOOSER_PACKAGE = '@voyaseek-ai/dsh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@voyaseek-ai/dsh-host-directory-picker-native',
  '@voyaseek-ai/dsh-host-directory-picker-browse',
  '@voyaseek-ai/dsh-client-ui-directory-picker-browse',
  '@voyaseek-ai/dsh-client-ui-directory-picker-native',
]
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if ((value.group === true || value.name === '@voyaseek-ai/cordis-plugin-group') && isUnknownArray(value.config)) {
  ...
  if (value.name !== '@voyaseek-ai/cordis-plugin-include') return
