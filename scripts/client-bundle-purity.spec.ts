...
function clientConfigs(id = '@voyaseek-ai/dsh-client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@voyaseek-ai/dsh-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('@voyaseek-ai/dsh-client-ui-slots')).toBeNull()
    expect(resolveId('@voyaseek-ai/dsh-client-web-react')).toBeNull()
    expect(resolveId('@voyaseek-ai/dsh-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('@voyaseek-ai/dsh-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@voyaseek-ai/dsh-host-apiproxy/api')).toBeNull()
    expect(resolveId('@voyaseek-ai/dsh-aistaff-employee-experience')).toBeNull()
    expect(resolveId('@voyaseek-ai/dsh-session/surface')).toBeNull()
    expect(resolveId('@voyaseek-ai/dsh-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@voyaseek-ai/dsh-goal/remote')).toBeNull()
    expect(() => resolveId('@voyaseek-ai/dsh-goal')).toThrow(/purity/)
    expect(() => resolveId('@voyaseek-ai/dsh-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@voyaseek-ai/dsh-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @voyaseek-ai leak', () => {
    expect(() => resolveId('@voyaseek-ai/dsh-agent')).toThrow(/purity/)
    expect(() => resolveId('@voyaseek-ai/dsh-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('@voyaseek-ai/dsh-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@voyaseek-ai/dsh-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@voyaseek-ai/dsh-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('@voyaseek-ai/dsh-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('@voyaseek-ai/') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['@voyaseek-ai/dsh-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@voyaseek-ai/dsh-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/@voyaseek-ai/dsh-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@voyaseek-ai/dsh-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@voyaseek-ai/dsh-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/@voyaseek-ai/dsh-client-connection/client.js.map')
