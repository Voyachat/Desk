...
const MERGE_HEAD = /declare module ['"](?:@voyaseek-ai\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@voyaseek-ai/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@voyaseek-ai/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@voyaseek-ai/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@voyaseek-ai/cordis'` Context merge
...
/**
 * Every event name a `declare module '@voyaseek-ai/cordis'` Events merge declares in one
