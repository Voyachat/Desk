/**
 * Rescope the vendored Cordis packages into the `@voyaseek-ai` scope, and undo
 ...
 * `@voyaseek-ai/dsh-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@voyaseek-ai/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@voyaseek-ai/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@voyaseek-ai/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@voyaseek-ai/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@voyaseek-ai/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@voyaseek-ai/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@voyaseek-ai/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@voyaseek-ai/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@voyaseek-ai/cordis-plugin-logger-console' },
]
...
const GENERIC_SKIPS: readonly GenericSkip[] = [
  ...
  // These browser-facing values are locale namespaces or trigger ids. They
  // deliberately remain `cordis`; only imports name the rescoped package.
  ...
  // Event documentation routes by the public `cordis/*` event prefix.
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@voyaseek-ai/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@voyaseek-ai/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@voyaseek-ai\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@voyaseek-ai/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@voyaseek-ai/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@voyaseek-ai/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@voyaseek-ai/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@voyaseek-ai/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@voyaseek-ai/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@voyaseek-ai` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@voyaseek-ai\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@voyaseek-ai/cordis']
  ...
  {
    // The rescoped name is already covered by the `@voyaseek-ai/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@voyaseek-ai/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@voyaseek-ai/')) {`,
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@voyaseek-ai/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@voyaseek-ai/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@voyaseek-ai/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@voyaseek-ai/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @voyaseek-ai, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@voyaseek-ai\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@voyaseek-ai/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@voyaseek-ai\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@voyaseek-ai/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @voyaseek-ai, so the consumer installs this repository's
...
function rewriteLine(line: string, file: string, all: readonly Pattern[]): string {
  ...
  for (const pattern of all) {
    ...
    out = out.replace(pattern.token, (match, quote: string, subpath: string) => {
      const cordisEvent = subpath === '/'
        || subpath === '/*'
        || /^\/(?:request-run(?:-resolved)?|dynamic-(?:package|retract)|inspect-query(?:-resolved)?)\\?$/.test(subpath)
      if (pattern.upstream === 'cordis' && cordisEvent) {
        return match
      }
