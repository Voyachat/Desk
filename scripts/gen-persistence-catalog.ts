...
const SESSION_PACKAGE = '@voyaseek-ai/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@voyaseek-ai/dsh-session/types'
 ...
 * top-level declaration (in `@voyaseek-ai/dsh-session`) and any declaration
 * merge inside a `declare module '@voyaseek-ai/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @voyaseek-ai/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
