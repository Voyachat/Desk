...
const WORKSPACE_ROOT_PACKAGE = '@voyaseek-ai/dsh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@voyaseek-ai/')) throw new Error(`${normalized} must name an @voyaseek-ai package`)
...
class DshFamily extends ReleaseFamily {
  ...
  readonly installedEntry = { packageName: '@voyaseek-ai/dsh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@voyaseek-ai/', '')}-v`
  }
