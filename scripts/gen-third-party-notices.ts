...
const PYTHON_METADATA: Record<string, { license: string; repo: string; role: string }> = {
  pydantic: { license: 'MIT', repo: 'https://github.com/pydantic/pydantic', role: 'runtime dependency of `voyaseek-harness-sdk`' },
...
/** Direct Rust dependencies linked into the packaged AiDesktop Supervisor. */
const RUST_METADATA: Record<string, { license: string; repo: string }> = {
  'aes-gcm': { license: 'Apache-2.0 OR MIT', repo: 'https://github.com/RustCrypto/AEADs' },
  'apple-native-keyring-store': { license: 'MIT OR Apache-2.0', repo: 'https://github.com/open-source-cooperative/apple-native-keyring-store' },
  base64: { license: 'MIT OR Apache-2.0', repo: 'https://github.com/marshallpierce/rust-base64' },
  'cap-fs-ext': { license: 'Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT', repo: 'https://github.com/bytecodealliance/cap-std' },
  'cap-std': { license: 'Apache-2.0 WITH LLVM-exception OR Apache-2.0 OR MIT', repo: 'https://github.com/bytecodealliance/cap-std' },
  'command-group': { license: 'Apache-2.0 OR MIT', repo: 'https://github.com/watchexec/command-group' },
  getrandom: { license: 'MIT OR Apache-2.0', repo: 'https://github.com/rust-random/getrandom' },
  'keyring-core': { license: 'MIT OR Apache-2.0', repo: 'https://github.com/open-source-cooperative/keyring-core' },
  libloading: { license: 'ISC', repo: 'https://github.com/nagisa/rust_libloading' },
  rusqlite: { license: 'MIT', repo: 'https://github.com/rusqlite/rusqlite' },
  serde: { license: 'MIT OR Apache-2.0', repo: 'https://github.com/serde-rs/serde' },
  serde_json: { license: 'MIT OR Apache-2.0', repo: 'https://github.com/serde-rs/json' },
  sha2: { license: 'MIT OR Apache-2.0', repo: 'https://github.com/RustCrypto/hashes' },
  subtle: { license: 'BSD-3-Clause', repo: 'https://github.com/dalek-cryptography/subtle' },
  'windows-native-keyring-store': { license: 'MIT OR Apache-2.0', repo: 'https://github.com/open-source-cooperative/windows-native-keyring-store' },
  'windows-sys': { license: 'MIT OR Apache-2.0', repo: 'https://github.com/microsoft/windows-rs' },
  zeroize: { license: 'Apache-2.0 OR MIT', repo: 'https://github.com/RustCrypto/utils' },
}

/** One Rust dependency disclosed in the generated notices. */
interface RustDep {
  name: string
  license: string
  repo: string
...
/**
 * Read direct dependencies from ordinary and target-specific Cargo tables.
 * @param source - Cargo.toml source.
...
function renderNonPermissiveNote(deps: ExternalDep[]): string {
  ...
  return `\n${subject} ${named.length === 1 ? 'runs' : 'run'} only as development tooling; their code is not linked into or distributed with any Voyaseek Harness artifact.\n`
}
...
export function render(): string {
  ...
  const nonPermissiveRust = rust.filter(dep => !isPermissive(dep.license))
  if (nonPermissiveRust.length > 0) {
    throw new Error(`gen-third-party-notices: packaged Rust dependency ${nonPermissiveRust.map(dep => `${dep.name} (${dep.license})`).join(', ')} is not a permissive license; review the distribution terms before regenerating.`)
  }
...
The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \`@voyaseek-ai\` scope. All are MIT-licensed; each directory preserves its upstream \`LICENSE\` file. Exact upstream commits and local modifications are recorded in [\`vendor/README.md\`](vendor/README.md).
...
## Packaged Rust Supervisor dependencies

Direct Rust dependencies linked into the AI Staff Supervisor sidecar. Exact direct and transitive versions are frozen by its Cargo lockfile. The \`rusqlite\` \`bundled\` feature compiles SQLite into the sidecar; SQLite itself is [public domain](https://sqlite.org/copyright.html).

| Crate | License |
| --- | --- |
${rust.map(dep => `| [\`${dep.name}\`](${dep.repo}) | ${dep.license} |`).join('\n')}
...
\`@voyaseek-ai/node-addon-landlock-run\` (and its platform packages) is built and released from this repository under BSD 3-Clause. It is listed here for completeness; it is first-party, not third-party.
`
}
