...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  const isPrivateAistaffMember = dir.startsWith('packages/aistaff/')
    || dir.startsWith('apps/aistaff-')
    || dir === 'apps/frontend-demo'
  ...
  } else if (isPrivateAistaffMember) {
    if (manifest.private !== true) {
      errors.push(`${label}: Aistaff product package must set "private": true`)
    }
