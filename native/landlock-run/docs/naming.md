...
The public package family belongs to the `@voyaseek-ai` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@voyaseek-ai/node-addon-landlock-run
@voyaseek-ai/node-addon-landlock-run-<platform>
```
