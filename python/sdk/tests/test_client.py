...
from voyaseek_harness import VoyaseekHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
""".strip()
    ...
    with VoyaseekHarness(launch_args_override=(sys.executable, str(script)), cwd=str(tmp_path)) as harness:
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from voyaseek_harness import VoyaseekHarnessConfig, Session
    ...
    assert "profile" not in inspect.signature(VoyaseekHarness.run).parameters
    assert "profile" not in inspect.signature(Session.run).parameters
    assert "system_prompt" not in VoyaseekHarnessConfig.__dataclass_fields__
    assert "max_tokens" in VoyaseekHarnessConfig.__dataclass_fields__
...
""".strip()
    ...
    module_dir = tmp_path / "voyaseek_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "voyaseek_harness_runtime", raising=False)
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install voyaseek-harness-runtime-bin"):
        HarnessClient().start()
