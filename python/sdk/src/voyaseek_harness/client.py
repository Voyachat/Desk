...
class HarnessConfig:
    """Configuration for launching the local Voyaseek Harness SDK runtime."""
...
class HarnessClient:
    """Synchronous JSON-RPC client for the Voyaseek Harness SDK runtime over stdio."""
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("Voyaseek Harness runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for Voyaseek Harness runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("Voyaseek Harness runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to Voyaseek Harness runtime") from exc
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("Voyaseek Harness runtime stdout closed"))
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            from voyaseek_harness_runtime import resolve_bundled_launch_args
        except ImportError as exc:
            raise FileNotFoundError(
                "Unable to locate the bundled Voyaseek Harness SDK runtime. "
                "Install voyaseek-harness-runtime-bin or set HarnessConfig.runtime_bin."
            ) from exc
    ...
    def _inject_bundled_default_config(self, env: dict[str, str]) -> None:
        ...
        from voyaseek_harness_runtime import bundled_default_config_path
