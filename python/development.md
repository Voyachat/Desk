...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `voyaseek-harness-runtime-bin` version.
...
pip install --find-links dist-python voyaseek-harness-sdk=="$version"
```
