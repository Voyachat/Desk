...
import { resolvePwshPath } from '@voyaseek-ai/dsh-pwsh-local'
import {
...
// The overlay swaps the shipped bash executor for @voyaseek-ai/dsh-pwsh-local;
...
describe.skipIf(MODE === 'record' || !HAS_PWSH)('web e2e: pwsh calls use the bash terminal-card layout', () => {
  ...
  it('renders the seeded pwsh call as a terminal card with the parsed exit pill', async () => {
    ...
    // shell-family variant, and the terminal card lives in the expanded body. The settled
    // row also sits inside the turn's collapsed activity fold on cold resume.
