...
import { deriveReplayScript, parseSessionLog } from '@voyaseek-ai/dsh-llm-replay'
import type { ReplayOverrideDoc } from '@voyaseek-ai/dsh-llm-replay'
...
const QUOTA_PROVIDER_MESSAGE = '{"error":{"code":429,"message":"quota for key AIza-live-secret exhausted"}}'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  it.skipIf(MODE === 'record')('cancels a hung stream deterministically via the readyFile marker', async () => {
    ...
    await expect.poll(
      () => page.getByRole('status').filter({ hasText: 'Voyaseek thinking' }).isVisible(),
      { timeout: 10_000 },
    ).toBe(true)
  ...
  it.skipIf(MODE === 'record')('surfaces a non-retryable AUTH failure without retrying', async () => {
    ...
    const errorStatus = page.getByRole('status').filter({ hasText: 'Model authentication failed' })
    await errorStatus.waitFor({ timeout: 10_000 })
    expect(await errorStatus.textContent()).toContain('update them, and retry')
    expect(await page.locator('body').textContent()).not.toContain('sk-preview-secret')
    const diagnostic = page.locator('[data-turn-error-category="auth"] details')
    expect(await diagnostic.getAttribute('open')).toBeNull()
    const snapshot = await captureStableAria(page, '[class*="centerCol"]', scaffold!.workspaceCwd)
    await compareOrRefreshGolden(ERROR_EXPECTED, snapshot, MODE)
    await diagnostic.locator('summary').click()
    expect(await diagnostic.textContent()).toContain('Code: AUTH')
    ...
    await page.getByText('Code: AUTH', { exact: true }).waitFor({ timeout: 10_000 })
    expect(await page.locator('body').textContent()).not.toContain('sk-preview-secret')
    expect(tripwire.pageErrors).toEqual([])
    expect(tripwire.warnings).toEqual([])
  }, 120_000)

  it.skipIf(MODE === 'record')('surfaces a raw 429 quota failure as safe actionable copy', async () => {
    await launch(() => ({
      patches: [{ at: 0, entry: { kind: 'throw', chunks: [], message: QUOTA_PROVIDER_MESSAGE, code: 'QUOTA' } }],
    }))
    onTestFailed(() => saveFailureShot(page, 'web-e2e-error-quota'))
    const { settled } = await sendPrompt()
    await settled
    expect(turnEndReasons(sessionEvents).at(-1)).toBe('error')
    expect(sessionEvents.filter(e => e.type === 'llm/retry')).toHaveLength(0)

    const errorStatus = page.getByRole('status').filter({ hasText: 'Model quota exhausted' })
    await errorStatus.waitFor({ timeout: 10_000 })
    expect(await errorStatus.textContent()).toContain('retry')
    expect(await page.locator('body').textContent()).not.toContain('AIza-live-secret')
    expect(await page.locator('body').textContent()).not.toContain(QUOTA_PROVIDER_MESSAGE)
    const diagnostic = page.locator('[data-turn-error-category="quota"] details')
    expect(await diagnostic.getAttribute('open')).toBeNull()
    await diagnostic.locator('summary').click()
    expect(await diagnostic.textContent()).toContain('Code: QUOTA')
    expect(tripwire.pageErrors).toEqual([])
    expect(tripwire.warnings).toEqual([])
  }, 120_000)
