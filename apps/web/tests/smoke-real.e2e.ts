    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          VOYASEEK_HOME: join(sessionsDir, '.voyaseek'),
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--port', '0'],
      {
        ...
        env: {
          ...
          VOYASEEK_HOME: join(workspace, '.voyaseek'),
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  it('empty-state first send completes a real model round', async () => {
    ...
    // This scenario spawns its own server against a fresh $VOYASEEK_HOME with the
    ...
    await page.waitForFunction(
      () => document.title !== 'Voyaseek' && document.title.endsWith(' — Voyaseek'),
      undefined,
      { timeout: 15_000 },
    )
    ...
    await page.waitForFunction(
      expected => document.title === `${expected} — Voyaseek`,
      durableTitle,
      { timeout: 15_000 },
    )
