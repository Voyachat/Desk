...
describe('web e2e: message IconActions and clocks on settled history', () => {
  ...
  it.skipIf(MODE === 'record')('copies the exact assistant message when the async API is denied', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-message-actions-copy'))
    const assistantTail = page.locator('[data-turn-tail="2"]')
    await assistantTail.waitFor({ timeout: 10_000 })
    await assistantTail.hover()
    // Mirror the desktop permission policy: Chromium exposes the async API,
    // but the host rejects its write. The visible click must then use the
    // real DOM copy command; granting clipboard-write here would hide the bug.
    await page.evaluate(() => {
      Object.defineProperty(navigator.clipboard, 'writeText', {
        configurable: true,
        value: () => Promise.reject(new DOMException('Denied by host', 'NotAllowedError')),
      })
    })
    await assistantTail.getByRole('button', { name: 'Copy', exact: true }).click()
    await expect.poll(() => assistantTail.getByRole('button', { name: 'Copied', exact: true }).count())
      .toBe(1)
    ...
    await expect.poll(() => page.evaluate(() => navigator.clipboard.readText()), { timeout: 5_000 })
      .toBe('DONE')
    await expect.poll(() => assistantTail.getByRole('button', { name: 'Copy', exact: true }).count(), { timeout: 2_000 })
      .toBe(1)
    await page.getByRole('button', { name: 'Settings' }).hover()
    await expect.poll(() => page.getByRole('tooltip').count(), { timeout: 2_000 }).toBe(0)
  }, 60_000)
