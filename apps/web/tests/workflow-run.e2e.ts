...
import type { Session, SessionEvent, SessionId } from '@voyaseek-ai/dsh-session'
import {
...
describe.skipIf(MODE === 'record')('web e2e: durable workflow run in Chat', () => {
  ...
  it('rebuilds the terminal record from history after reload', async () => {
    ...
    await page.waitForSelector('[class*="frame"]', { timeout: 30_000 })
    const dashboard = page.getByRole('button', { name: '1 workflow' })
    await dashboard.waitFor({ timeout: 15_000 })
    await dashboard.click()
    const dashboardDialog = page.getByRole('dialog', { name: 'Current session workflow dashboard' })
    await dashboardDialog.waitFor()
    const dashboardRun = dashboardDialog.getByRole('button', { name: /snapshot-flow.*1 step.*Succeeded/ })
    await dashboardRun.waitFor()
    await dashboardRun.click()
    await dashboardDialog.getByText(/Reply with exactly the word WF_CHILD_OK/).waitFor()
    await dashboard.click()
    ...
    await phase.click()
    await page.getByText(CHILD_PROMPT, { exact: false }).waitFor()
    await page.getByRole('button', { name: 'Inspect call' }).waitFor()
