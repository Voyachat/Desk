...
describe('web e2e: queue row actions', () => {
  ...
  it.skipIf(MODE === 'record')('settles Todo projection when a model turn fails', async () => {
    ...
    session.append('todo/write', {
      todos: [
        { content: 'Delivered result', status: 'completed' },
        { content: 'Active implementation', status: 'in_progress' },
        { content: 'Later validation', status: 'pending' },
      ],
    })
    session.append('turn/end', {
      ...
      reason: {
        ...
        error: {
          ...
          requestId: ProviderRequestId('req_quota_429'),
          message: '{"error":{"message":"quota for key AIza-web-secret exhausted"}}',
        },
      },
    })

    await page.getByText('1 completed · 1 failed · 1 blocked (not run this turn)', { exact: true })
      .waitFor({ timeout: 10_000 })
    await page.getByRole('button', { name: /To-dos/ }).click()
    const statuses = await page.locator('[data-testid="todo-panel"] [data-status]')
      .evaluateAll(items => items.map(item => item.getAttribute('data-status')))
    expect(statuses).toEqual(['completed', 'failed', 'blocked'])
    ...
    expect(await diagnostic.textContent()).toContain('Code: QUOTA · HTTP: 429 · Retry-After: 2000ms')
    expect(await page.locator('body').textContent()).not.toContain('AIza-web-secret')
