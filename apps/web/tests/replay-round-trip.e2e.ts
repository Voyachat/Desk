...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it('records the browser language as model context for replies and deliverables', () => {
    if (settledSessionId === undefined) throw new Error('the drive turn did not publish a session id')
    const agent = scaffold.ctx.agents.get(settledSessionId)
    if (agent === undefined) throw new Error(`the settled Web agent ${settledSessionId} is no longer live`)
    ...
    if (snapshot?.type !== 'user/message' || snapshot.data.source.kind !== 'plugin'
      || snapshot.data.source.form !== 'snapshot') {
      throw new Error('the settled Web request has no durable locale context')
    }
    expect(snapshot.data.source.sections.find(section => section.name === 'user:locale')?.text)
