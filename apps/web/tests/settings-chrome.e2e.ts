...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/locale:\n\s+preference: en/)
    expect((await scaffold.ctx.systemPrompt.assemble()).contexts
      .find(entry => entry.name === 'user:locale')?.text)
  ...
  it('adopts an English browser as the durable product and model language', async () => {
    // A fresh Host follows the browser, then persists that resolved choice so
    // the Host and later browsers consume the same product preference.
    ...
    try {
      ...
      await expect.poll(async () => readFile(join(fresh.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
        .toMatch(/locale:\n\s+preference: en/)
      expect((await fresh.ctx.systemPrompt.assemble()).contexts
        .find(entry => entry.name === 'user:locale')?.text)
        .toContain('English (BCP 47: en)')
