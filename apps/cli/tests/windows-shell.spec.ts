...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@voyaseek-ai/dsh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@voyaseek-ai/dsh-base', '@voyaseek-ai/dsh-web-app'])
    ...
    for (const name of ['@voyaseek-ai/dsh-pwsh-sandbox', '@voyaseek-ai/dsh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@voyaseek-ai/dsh-base'])
