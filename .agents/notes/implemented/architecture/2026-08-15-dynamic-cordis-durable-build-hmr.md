...
The cross-runtime development-HMR integration spec has its own `tsconfig.hybrid.json` compiler program. It intentionally loads both service faces, while the repository Host and Client aggregates remain isolated so their incompatible `Context.sessions` declarations cannot affect unrelated tests. The Client compiler phase runs this third program after Host Typert generation, because the integration spec reaches the Client remote graph.
