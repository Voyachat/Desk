...
At every flow size, scrolling into the top 320px auto-requests one older page per visit (`OLDER_AUTOLOAD_THRESHOLD_PX`), rearming when the reader leaves the zone; the prepend's own anchor shift pushes the position out of the zone, so settled pages never refire. The header button remains an explicit retry and accessibility path. Both virtualized and plain-flex flows use the same semantic anchor, so concurrent streaming and responsive reflow cannot move the row the reader was viewing while an older page arrives.
