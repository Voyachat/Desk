...
启用虚拟化时，滚动进入顶部 320px 会在每次进入时自动请求一页更早历史（`OLDER_AUTOLOAD_THRESHOLD_PX`），阅读者离开该区域后重新启用；prepend 自身的 anchor 位移会把位置推出该区域，所以已完成的页不会重复触发。未达到阈值的 flow 仍只通过 header 按钮分页，从而保持既有非虚拟化滚动规则。
