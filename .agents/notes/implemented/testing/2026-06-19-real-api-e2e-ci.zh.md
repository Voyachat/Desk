...
### 本地提供方变体补充 DeepSeek 工作流

在已授权的本机上，真实场景可以使用另一项已安装的模型提供方及其专用密钥，验证同一组装产品入口。付费本地变体必须显式拥有提供方重试、测试重试、响应 token 和模型步骤预算；步骤预算会在首个超额请求开始前取消运行。该证据用于补充提供方覆盖，但不替代专用 DeepSeek CI 工作流、其外部 API 或携带 secret 的触发策略。
