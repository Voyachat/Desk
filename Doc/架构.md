...
AiDesktop 当前以隔离复制的 DSH 本地源码快照作为派生基线；[adoption ledger](../.open-source/adoptions.yaml) 权威记录 `mode=source`、内容寻址快照引用、MIT 许可证、上游 URL、排除项和本地差异。参考工作区不是构建或运行依赖；官方精确 SHA 是未来升级证据的增强项，不是首个纵向代码的门槛。
...
`vendor/**`、`packages/core/**`、`packages/client/**` 通用 chrome、通用 API/CLI 和 `agent-loop` 默认保持近上游。若产品暴露通用扩展点缺口，只提交最小、无 Aistaff 语义、可上游化的补丁，并记录删除条件；不得用兼容层长期掩盖快照漂移。
...
DSH 升级从新的可审计源码候选开始：更新 adoption ledger，以当前采用快照到候选快照的范围 diff 审查 Boot/Profile、Session、Client slots、权限、Web carrier 和 Bundle，再重放最小通用补丁并运行产品纵向 smoke。取得官方精确 SHA 时一并登记；升级不能自动改变 Cloud wire 或本地数据语义。
